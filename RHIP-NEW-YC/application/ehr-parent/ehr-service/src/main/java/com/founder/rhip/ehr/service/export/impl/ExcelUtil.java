...
public class ExcelUtil {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < 60; i = i + 2) {
			...
			for (int j = 0; j < 25; j++) {
				...
				if (j == 0) {
					...
					cellStyle = ExcelUtil.setCellStyleAlignment(cellStyle, CellStyle.ALIGN_CENTER, CellStyle.VERTICAL_CENTER);
					cellStyle = ExcelUtil.setFillBackgroundColors(cellStyle, IndexedColors.ORANGE.getIndex(), IndexedColors.ORANGE.getIndex(), CellStyle.SOLID_FOREGROUND);
					cell.setCellStyle(cellStyle);
					cell.setCellValue(1);
				}
			}
		...
		// 输出
		OutputStream os = new FileOutputStream(new File("D://ExcelUtil.xls"));
		wb.write(os);
		os.close();
	}
	 ...
	 * @param width
	 */
	public static void SetColumnWidth(HSSFSheet donationSheet, int column, int width) {
		donationSheet.setColumnWidth(column, width);
	}
}

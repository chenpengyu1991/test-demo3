 ...
 */
@Repository("diStatisticsDao")
public class DiStatisticsDaoImpl extends AbstractDao<PersonInfo, Long> implements IDiStatisticsDao {
	...
	private String buildRecordSql(String orgType, String townCode, String centreCode, String stationCode, Date inputBeginDate, Date inputEndDate, String householdType, String gender) {
        ...
        String dmManageDate=buildManageWhereCondition(inputBeginDate, inputEndDate);
    ...
    private String buildManageWhereCondition(Date inputBeginDate, Date inputEndDate) {
        StringBuilder where = new StringBuilder();
        if (null != inputBeginDate) {
            where.append(" and DM.DI_MANAGED_DATE>=:inputBeginDate ");
        }
        if (null != inputEndDate) {
            where.append(" and DM.DI_MANAGED_DATE<=:inputEndDate ");
        }
        return where.toString();
    }
    ...
    public String getQueryByTown(){
        return "--糖尿病登记数\n" +
                ...
                "	B.DI_FOLLOWUP_COUNT,\n" +
                "	C.DI_MANAGED_COUNT,\n" +
                "	D .DI_MANAGED_LIMIT_COUNT,\n" +
                "	E .DI_FOLLOWUP_WELL_COUNT\n" +
                "FROM\n" +
                "	(\n" +
                ...
                "			DM.CREATE_GBCODE\n" +
                "		) TARGET_CODE,\n" +
                "		\"COUNT\" (1) DI_FOLLOWUP_COUNT\n" +
                "	FROM\n" +
                "		V_DM_DISEASE_INFO DM\n" +
                "	INNER JOIN DM_DIABETIC_FOLLOWUP DF ON DM.PERSON_ID = DF.PERSON_ID\n" +
                "	WHERE\n" +
                "		DM.DI_FLAG = '2'    %3$s %10$s\n" +
                "	GROUP BY\n" +
                "		ROLLUP (DM.CREATE_GBCODE)\n" +
                ...
                "		V_DM_DISEASE_INFO DM\n" +
                "	WHERE\n" +
                "		DM.DI_FLAG = '2'    %4$s %10$s\n" +
                "	AND EXISTS (\n" +
                "		SELECT\n" +
                "			2\n" +
                "		FROM\n" +
                "			DM_DIABETIC_FOLLOWUP DF\n" +
                "		WHERE\n" +
                ...
                "	--计算规范话随访\n" +
                "	SELECT\n" +
                "		\"COUNT\" (1) DI_MANAGED_LIMIT_COUNT,\n" +
                "		DECODE (\n" +
                ...
                "			COUNT (1) TOTAL\n" +
                "		FROM\n" +
                "			DM_DIABETIC_FOLLOWUP DF WHERE 1=1 %7$s\n" +
                "		GROUP BY\n" +
                "			DF.PERSON_ID\n" +
                ...
                "			DM.CREATE_GBCODE\n" +
                "		) TARGET_CODE,\n" +
                "		\"COUNT\" (1) DI_FOLLOWUP_WELL_COUNT\n" +
                "	FROM\n" +
                "		V_DM_DISEASE_INFO DM\n" +
                "	\n" +
                "	INNER JOIN DM_DIABETIC_FOLLOWUP DFL ON DM.PERSON_ID = DFL.PERSON_ID\n" +
                "	INNER JOIN (\n" +
                "		SELECT\n" +
                "			DF.PERSON_ID,\n" +
                "			\"MAX\" (DF.ID) ID\n" +
                "		FROM\n" +
                "			DM_DIABETIC_FOLLOWUP DF WHERE 1=1 %8$s\n" +
                "		GROUP BY\n" +
                "			DF.PERSON_ID\n" +
                "	) DF ON DF.PERSON_ID = DFL.PERSON_ID\n" +
                "	AND DFL.ID = DF.ID\n" +
                "	WHERE\n" +
                "		DFL.FPG < "+EHRConstants.BS_OK_VALUE[1]+" AND  DFL.FPG > "+EHRConstants.BS_OK_VALUE[0]+"\n" +
                "	AND 	DM.DI_FLAG = '2'    %9$s %10$s\n" +
                "	GROUP BY\n" +
                "		ROLLUP (DM.CREATE_GBCODE)\n" +

...
(function($) {
	...
	$.alerts = {
		...
		cancelButton : '&nbsp;&nbsp;取消&nbsp;&nbsp;', // text for the Cancel
		// button
	...
	jConfirm = function(message, callback) {
		$.alerts.confirm(message, "确认提示", callback);
	};

	backConfirm = function(message, callback) {
		$.alerts.confirm(message, "退出提示", callback);
		$("#popup_ok").val("　离开此页　");
		$("#popup_cancel").val("　留在此页　");
	};
...
var msgUtil = (function() {
	...
	function msgConfirm(message, callback,okBtn,cancelBtn){
		$.alerts.confirm(message, "确认提示", function(r){if(r){callback();}});
		if(!$.isEmpty(okBtn)){
			$("#popup_ok").val(okBtn);
		}
		if(!$.isEmpty(cancelBtn)){
			$("#popup_cancel").val(cancelBtn);
		}
	}

	function backConfirm(callback){
		$.alerts.confirm("确认离开？", "退出提示",  function(r){if(r){callback();}});
		$("#popup_ok").val("确定");
		$("#popup_cancel").val("取消");
	}

	function msgConfirmAll(message, callbackOk,callbackCancel){
		$.alerts.confirm(message, "确认提示", function(r){
			if(r){
				callbackOk();
			}else{
				callbackCancel();

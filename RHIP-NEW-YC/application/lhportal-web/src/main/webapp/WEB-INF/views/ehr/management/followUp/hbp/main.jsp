								...
								<fieldset>
									...
									<table class="posttable">
										...
										<tr>
											...
											<td><ehr:dic dicmeta="DMD00038" code="${hbp.curSymptom }" /> 
											<c:out value="${hbp.otherSymptom}" /></td>
										</tr>
									</table>
									<fieldset>
										<legend>体征</legend>
										<table class="posttable">
											...
											<tr>
												...
												<td><c:out value="${hbp.sbp}" />/<c:out value="${hbp.dbp}" /></td>
											</tr>
											<tr>
												<th><label  for="height">身高</label></th>
												<td><c:out value="${hbp.height}" /></td>
											</tr>
											<tr>
												<th><label  for="bodyWeight">体重</label></th>
												<td><c:out value="${hbp.bodyWeight}" />/
							    					<c:out value="${hbp.nextFollowupBodyWeight}"/>kg
							    				</td>
											</tr>
											<tr>
												<th><label  for="indexOfBodyCharacteristics">体质指数BMI</label></th>
												<td>
													<c:out value="${hbp.indexOfBodyCharacteristics}" />/
													<c:out  value="${hbp.nextFollowupBmi}" />
												</td>
											</tr>
											<tr>
									...
									<fieldset>
										...
										<table class="posttable">
											...
											<tr>
												...
												<td>
													<c:out value="${hbp.dailyDailySmokeber}" />/
													<c:out value="${hbp.smokeberTarget}" />
												</td>
											</tr>
											<tr>
												<th><label for=dailyDrink>日饮酒量（支）</label></th>
												<td>
													<c:out value="${hbp.dailyDrink}" />/
													<c:out value="${hbp.nextFollowupDailyDrink}" />
												</td>
											</tr>
											<tr>
												<th><label for="trainFrequency">运动</label></th>
												<td><c:out value="${hbp.trainFrequency}"/>次/周
													<c:out value="${hbp.trainingMin}" />分钟/次</td>
											</tr>
											<tr>
												<th></th>
												<td><c:out value="${hbp.nextExerciseFrequency}"/>次/周
													<c:out value="${hbp.nextExerciseTime}" />分钟/次</td>
											</tr>
											<tr>
												<th><label  >摄盐情况（克/天）</label></th>
												<td>
													<ehr:dic dicmeta="DMD00063" code="${hbp.salinity}"></ehr:dic> /
													<ehr:dic dicmeta="DMD00063" code="${hbp.nextSalinityTarget}"></ehr:dic>
												</td>
											</tr>
											<tr>
									...
									<fieldset>
										...
										<table class="posttable">
											...
											<tr>
												<th>总胆固醇</th>
												<td><c:out value="${hbp.tc}" />mmol/L</td>
											</tr>
											<tr>
												<th>甘油三酯</th>
												<td><c:out value="${hbp.triglycerideValue}"/>mmol/L</td>
											</tr>
											<tr>
												<th>血清低密度脂蛋白胆固醇</th>
												<td><c:out value="${hbp.ldlcDetectValue}" />mmol/L</td>
											</tr>
											<tr>
												<th>血清高密度脂蛋白胆固醇</th>
												<td><c:out value="${hbp.hdlcDetectValue}" />mmol/L</td>
											</tr>
										</table>
									</fieldset>
									...
									<table class="posttable">
										...
										<tr>
											...
											<td><ehr:dic dicmeta="FS10238" code="${hbp.sideEffects }"></ehr:dic></td>
										</tr>
										<tr>
								...
								<fieldset>
									...
									<table class="posttable" <c:if test='${hbp.referralHbpFlag eq "2" }'> style="display: none" </c:if>>
										...
										<tr>
											<th>机构</th>
											<td class="colinput">
												<c:out value="${hbp.referralOrganName}"/>
<!-- 												<ehr:org code="${hbp.referralOrganCode}"/> -->
											</td>
										</tr>
										<tr>
											<th>科别</th>

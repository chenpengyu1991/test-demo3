...
/**
 * (镇、疾病控制中心、综合医院、卫生监督检验(监测、检测)所、卫计委、其他) 中心 站 等机构的级联下拉列表
 *此标签在TownCentreStationTag基础上修改,原有的dic-town-centre-station不做修改,该标签只用于 医务人员和用户管理模块
 */
public class MedicalOrganTag extends BaseTag {
	...
	private Boolean isShowJk = false;// 是否显示疾控 默认不显示
	private Boolean shoQwjw = false;// 是否显示区卫计委 默认不显示
	...
	public int doStartTagInternal() throws JspTagException {
		...
		if (isShowJk) {
			html.append(getOption(this.getDicItem("_showJk","疾病预防控制中心")));
		}
		if (shoQwjw) {
			html.append(getOption(this.getDicItem("_showWjw","区卫计委")));
		}
		if (isShow) {
			...
			if(ObjectUtil.isNotEmpty(this.stationName)) {
                ...
                if((isAuthorize && (ObjectUtil.equals(currentOrg.getGenreCode(), OrgGenreCode.JK.getValue())||
                    ...
                    boolean isNotSecendSelect = isAuthorize && (ObjectUtil.equals(currentOrg.getGenreCode(), OrgGenreCode.CENTRE.getValue())||
                            ObjectUtil.equals(currentOrg.getGenreCode(), OrgGenreCode.INSTITUTES.getValue()));
                    boolean stationDisplay = (StringUtils.isNotEmpty(this.centreValue)&& (ObjectUtil.equals(currentOrg.getGenreCode(), OrgGenreCode.CENTRE.getValue()))||StringUtils.isNotEmpty(this.stationValue) || isNotSecendSelect) ? false : true;
	...
	private StringBuffer getCentre(String gbCode, Organization currentOrg) {
		...
		if(EHRConstants._SHOWJK.equals(gbCode)){//疾病预防控制中心
			criteria.add("GENRE_CODE", OrgGenreCode.JK.getValue());
		}else if(EHRConstants._QWJW.equals(gbCode)){//区卫计委
			criteria.add("GENRE_CODE", OrgGenreCode.AREA_HEALTH.getValue());
		}else if(gbCode.equals("_hospital")){
	...
	public Boolean getIsShowJk() {
		return isShowJk;
	}

	public void setIsShowJk(Boolean isShowJk) {
		this.isShowJk = isShowJk;
	}

	public Boolean getShoQwjw() {
		return shoQwjw;
	}

	public void setShoQwjw(Boolean shoQwjw) {
		this.shoQwjw = shoQwjw;
	}
}

...
public class DictionaryListTag extends BaseTag {
    ...
    private List<DicItem> getDicItems() {
    	List<DicItem> temp =null;
    	if (ObjectUtil.isNotEmpty(parentCode)) {
    		 temp = dictionaryApp.queryDicItems(dicmeta, parentCode);
		}else {
			 temp = dictionaryApp.queryDicItem(dicmeta);// WebCache.dicItemList.get(dicmeta);
		}
	...
	private StringBuilder getSelectBefore () {
		...
		sb.append(" onchange=\"showTitle('" + random + "');");
		...
		if (StringUtils.isNotEmpty(cssClass)) {
			sb.append(" class=\"" + cssClass.replaceAll("\"", "'") + "\"");
		}
	...
	private StringBuilder getMultipleOption(DicItem dic, StringBuilder sb) {
		...
		for(String temp: values) {
			if (dic.getItemCode().equals(temp)) {
				sb.append("\t\t<option title=\""+ dic.getItemName() + "\" value=\"" + dic.getItemCode() + "\" selected>" + dic.getItemName() + "</option>\r\n");
				flag = false;
				break;
			}
		}
		if (flag){
			sb.append("\t\t<option title=\""+ dic.getItemName() + "\" value=\"" + dic.getItemCode() + "\">" + dic.getItemName() + "</option>\r\n");
		}
		return sb;
	}
	...
	private String showTitle(String value) {
		if(StringUtil.isNotEmpty(value)) {
			DicItem item = dictionaryApp.queryDicItem(dicmeta, value);
			if (ObjectUtil.isNotEmpty(item)) {
				return " title=\"" + item.getItemName() + "\"";
			} else {

...
public class QueryForm implements Serializable {
	...
	/**
	 *
	 * @param isConvertDiseaseType 慢病类型是否需要转化
	 * @param flagValue 若是已撤销值应该为1 若是未撤销应为2
	 * @return
	 */
	public Criteria toCriteria(boolean isConvertDiseaseType, String flagValue) {
...
//
		if (StringUtil.isNotEmpty(diseaseType)) {
					...
					for (String type : types) {
						switch (type) {
							case EHRConstants.DM_HBP_TYPE:
								disTypeCriteria.add(LOP.OR, "hbpFlag", flagValue);
								break;
							case EHRConstants.DM_DI_TYPE:
								disTypeCriteria.add(LOP.OR, "diFlag", flagValue);
								break;
							case EHRConstants.DM_CORONARY_TYPE:
								disTypeCriteria.add(LOP.OR, "coronaryFlag", flagValue);
								break;
							case EHRConstants.DM_TUMOR_TYPE:
								disTypeCriteria.add(LOP.OR, "tumorFlag", flagValue);
								break;
							case EHRConstants.DM_STROKE_TYPE:
								disTypeCriteria.add(LOP.OR, "strokeFlag", flagValue);
								break;
						}
					}
				} else {
			...
			if (isConvertDiseaseType) {
				...
				criteriaFlag.add("dmDiseaseInfo.hbp_flag", flagValue);
				criteriaFlag.add(LOP.OR, "dmDiseaseInfo.di_flag", flagValue);
				criteriaFlag.add(LOP.OR, "dmDiseaseInfo.stroke_flag", flagValue);
				criteriaFlag.add(LOP.OR, "dmDiseaseInfo.coronary_flag", flagValue);
				criteriaFlag.add(LOP.OR, "dmDiseaseInfo.tumor_flag", flagValue);
				criteria.add(criteriaFlag);
			}
		}
...
//				criteria.add(dateParam);
				...
				criteria.add("phyExamType", 1);
//				criteria.add("dmDiseaseInfo.LAST_PHY_EXAMINATION_YEAR", OP.GE, year);
			}
			//体检时间
			if(null != yearDt && yearDt != ""){
				criteria.add("yearDt", yearDt);
			}else{
				criteria.add("yearDt", year);
			}
		}

		//建卡时间
		if(null != yearDt && yearDt != "" && ObjectUtil.isNotEmpty(phyExamType)){
			if (StringUtil.isNotEmpty(diseaseType)) {
				String[] types = diseaseType.split(",");
				Criteria disTypeCriteria = new Criteria();
				criteria.add(disTypeCriteria);
				if (types.length > 0) {
					for (String type : types) {
						switch (type) {
						case EHRConstants.DM_HBP_TYPE:
							disTypeCriteria.add(LOP.OR, "dmDiseaseInfo.HBP_MANAGED_DATE", OP.LE, DateUtil.parseDateString(yearDt+""+"/12/31"));
							break;
						case EHRConstants.DM_DI_TYPE:
							disTypeCriteria.add(LOP.OR, "dmDiseaseInfo.DI_MANAGED_DATE", OP.LE, DateUtil.parseDateString(yearDt+""+"/12/31"));
							break;
						default:
							disTypeCriteria.add(LOP.OR, "dmDiseaseInfo.HBP_MANAGED_DATE", OP.LE, DateUtil.parseDateString(yearDt+""+"/12/31"));
							disTypeCriteria.add(LOP.OR, "dmDiseaseInfo.DI_MANAGED_DATE", OP.LE, DateUtil.parseDateString(yearDt+""+"/12/31"));

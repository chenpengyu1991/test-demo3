	...
	<div class="table-basic">
		...
		<table style="line-height: 10px;" class="posttable">
			...
			<tr>
				...
				<th>是否以新生儿身份纳入管理</th>	
				<td>
				<c:out value='${neonatalFamilyVisit.managementFlag eq "2" ? "是" : "否"}'/>			
				</td>			
			...
			<tr>
				...
				<td>
					<c:if test="${!empty neonatalFamilyVisit.deformitySign}">
						<c:out value='${fn:trim(neonatalFamilyVisit.deformitySign) eq "1" ? "无" : "有"}'/>
						<c:if test="${fn:trim(neonatalFamilyVisit.deformitySign) eq '2'}"> 
			...
			<tr>
				...
				<td colspan="3">
					<ehr:dic code="${neonatalFamilyVisit.diseaseScreeningProject}" dicmeta="CV0450008"/>
						<input type="hidden" id="diseaseScreeningProject" value="${neonatalFamilyVisit.diseaseScreeningProject}" />
			...
			<tr>
				...
				<td colspan="3">
					<c:if test="${!empty neonatalFamilyVisit.skinVisionInspection}">
						<c:choose>
							<c:when test="${neonatalFamilyVisit.skinVisionInspection eq '01'}">未见异常</c:when>
			...
			<tr>
				...
				<td colspan="3">
					<c:if test="${!empty neonatalFamilyVisit.abnormalChestAnomaly}">
						<c:choose>
							<c:when test="${neonatalFamilyVisit.abnormalChestAnomaly eq '0'}">未见异常</c:when>

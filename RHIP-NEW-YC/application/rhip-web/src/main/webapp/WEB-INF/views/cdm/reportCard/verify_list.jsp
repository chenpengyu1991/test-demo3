				...
				<colgroup>
					...
					<tbody>
						<tr>
							...
							<td class="colinput">
							<tag:idcardInput name="idcard"  style="ime-mode:Disabled;" id="idcard" cssClass="x-layui-input"></tag:idcardInput>
					...
					<tr>
						...
						<td class="colinput">
							...
							<ehr:authorize ifAnyGranted="0307">
								<label><input <c:if test="${reportStatus eq 1 or reportStatus eq null}">checked="checked"</c:if> type="radio" id="dm-report-status" name="reportStatus" class="radioGroup" value="1,2">
								待审批</label>
								<%-- input type="radio" id="dm-report-status" name="reportStatus" class="radioGroup"  value="2">
									<label>退回</label> --%>
							</ehr:authorize> 
							...
							<ehr:authorize ifAnyGranted="0207">
								...
								<%-- input type="radio" id="dm-report-status" name="reportStatus" class="radioGroup"  value="2">
									<label>退回</label> --%>
								<label><input <c:if test="${reportStatus eq 2}">checked="checked"</c:if> type="radio" id="dm-report-status" name="reportStatus" class="radioGroup" value="4,8,12">
								待分配</label>
								<%--<input type="radio" id="dm-report-status" name="reportStatus" class="radioGroup" value="8">
									<label>分配退回</label>--%>
								<%--<ehr:dic-radio id="dm-report-status"   dicmeta="DMD00005"  uninclude="3,5,6,7,9,10,11" name="reportStatus"   /> --%>
							</ehr:authorize> 
							
							<ehr:authorize ifAnyGranted="0107,01">
								<label><input type="radio" <c:if test="${reportStatus eq 1 or reportStatus eq null}">checked="checked"</c:if> name="reportStatus" class="radioGroup" value="3">
								待审批</label>
								<label><input <c:if test="${reportStatus eq 2}">checked="checked"</c:if> type="radio" name="reportStatus" class="radioGroup" value="6">
								待分配</label>
								<%-- <ehr:dic-radio  id="dm-report-status"   dicmeta="DMD00005" uninclude="1,2,4,5,7,8,9,10,11"  name="reportStatus"   /> --%>
							</ehr:authorize>
						</td>
						<td class="coltext"><ehr:authorize ifAnyGranted="0107">上报机构 </ehr:authorize></td>
						<td class="colinput"><ehr:authorize ifAnyGranted="0107">

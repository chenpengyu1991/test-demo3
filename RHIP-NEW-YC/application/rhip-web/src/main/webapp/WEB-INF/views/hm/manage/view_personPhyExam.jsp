...
<table class="posttable">
		...
		<td>
			<%-- <c:choose>
				<c:when test="${not empty examination.examYear}">
					<tag:dateInput name="examYear" date="${examination.examYearDate}" pattern="yyyy" style="width:100px;"/>
				</c:when>
				<c:when test="${not empty examination.examinationDate}">
					<tag:dateInput name="examYear" date="${examination.examinationDate}" pattern="yyyy" style="width:100px;"/>
				</c:when>
				<c:otherwise>
					<tag:dateInput name="examYear" date="${today}" pattern="yyyy" style="width:100px;"/>
				</c:otherwise>
			</c:choose> --%>
		</td>
		<th>编号 </th>
		<td >
			<c:choose>
				<c:when test="${not empty examination.physicalExamCode}">
					<input style="width:130px;" type="text" name='physicalExamCode' value="${examination.physicalExamCode}" disabled="disabled"/>
				</c:when>
				<c:otherwise>
					<input style="width:130px;" type="text" value="自动生成" disabled="disabled"/>
				</c:otherwise>
			</c:choose>
		</td>
	</tr>
	<tr>
		...
		<td>
			<ehr:org code="${examination.examinationOrganCode}"/>
		</td>
		<th><label class="required">体检日期</label></th>
		<td>
			<c:if test="${empty examination.examinationDate}">
				<tag:dateInput style="width:120px;"  reg="{'required':true}" onlypast="true" id="examinationDate_input" date="${today}" name="examinationDate" cssClass="width100"></tag:dateInput>
			</c:if> 
			<c:if test="${not empty examination.examinationDate}">
				<tag:dateInput style="width:120px;" reg="{'required':true}"  onlypast="true" id="examinationDate_input" date="${examination.examinationDate}" name="examinationDate" cssClass="width100"></tag:dateInput>
			</c:if>
		</td>
		<th>责任医生</th>
		<ehr:authorize ifNotGranted="01">
			<td>
				<ehr:staff-list name="manaDoctorId" value="${examination.manaDoctorId}" style="width:130px" defaultval="Y" orgCode="" ></ehr:staff-list>
			</td>
		</ehr:authorize>
		<c:if test="${not empty staffName}">
			<ehr:authorize ifAnyGranted="01">
				<td><input  type= "text" value="${staffName}" readonly="readonly"/></td>
			</ehr:authorize>
		</c:if>
	</tr>
</table>
	...
	<fieldset class="layui-elem-field">
		...
		<table class="posttable">
			...
			<tr>
				...
				<td>
					左侧
					<tag:numberInput name="leftSbp" value="${examination.leftSbp}"  style="width: 40px" reg="{'min':0,'max':9999}"/>/
					<tag:numberInput name="leftDbp" value="${examination.leftDbp}"  style="width: 40px" reg="{'min':0,'max':9999}"/>mmHg
					右侧
					<tag:numberInput  name="pastHighestSbp" value="${examination.rightSbp}" style="width: 40px" reg="{'min':0,'max':9999}"/>/
					<tag:numberInput  name="pastHighesDbp" value="${examination.rightDbp}"  style="width: 40px"  reg="{'min':0,'max':9999}"/>mmHg
			...
			<fieldset class="layui-elem-field">
				...
				<table class="posttable" >
					<tr>
						<th width="16%">锻炼频率</th>
						<td colspan="2">
							<ehr:dic-radio name="trainFrequencyTypeCode" dicmeta="FS10208" value="${examination.trainFrequencyTypeCode}"  onchange="hmPersonPhyExamAdd.setContentStyle(this,'exerciseTypeSpan:keepTimeSpan:exerciseTimeSpan')"/>
						</td>
					</tr>
					<tr>
						<th>每次锻炼时间</th>
				    	...
				    	<span id="keepTimeSpan">
				        	<tag:numberInput reg="{'min':0,'max':99999}" name="trainingTotaltime" value="${examination.trainingTotaltime}" style="width: 80px"/>年
				        </span>
						</td>
					</tr>
					<tr>
			...
			<fieldset class="layui-elem-field">
				...
				<table class="posttable">
					...
					<tr>
						<th width="16%">吸烟状况</th>
						<td >
							<ehr:dic-radio name="smodeStatusCode" dicmeta="CV0300101" value="${examination.smodeStatusCode}" onchange="hmPersonPhyExamAdd.setContentStyle(this,'smokePerDaySpan:beginSmokeAgeSpan:endSmokeAgeSpan')"/>
						</td>
					</tr>
					<tr>
						<th>日吸烟量</th>
			...
			<fieldset class="layui-elem-field">
				...
				<table class="posttable">
					...
					<tr>
						<th width="16%">饮酒频率</th>
						<td >
							<ehr:dic-radio name="drinkFrequency" value="${examination.drinkFrequency }" dicmeta="CV0300104"  onchange="hmPersonPhyExamAdd.setContentStyle(this,'wineTypeSpan:isDrunkSpan:beginDrinkingAgeSpan:isEndDrinkingSpan:drinkingPerDaySpan')" code="1,2,3,4"/>
						</td>
					</tr>
					<tr>
						<th>日饮酒量</th>
					...
					<table>
						...
						<tr>
							<td colspan="4">齿列&nbsp;
								...
								<div id="dentitionAnomalyDiv" style="display: none;width: 100%; padding-left: 32px;">
...
<tr>
				...
				<td><label><input id="dreWJ" type="radio" name="dreCheckResultType" ${examination.dreCheckResultType eq "0" ? "checked" : ""}
						value="0" onchange="toggleOther('dreCheckResultType','dreCheckResultTypeSpan',9)" />未检</label>
					<ehr:dic-radio id="dre" name="dreCheckResultType" dicmeta="CV0410013"
						value="${examination.dreCheckResultType}" onchange="toggleOther('dreCheckResultType','dreCheckResultTypeSpan',9)" />
					<span style="display: none;" id="dreCheckResultTypeSpan"> 描述:<input reg='{"maxlength":"33"}' type="text"
	...
	<fieldset class="layui-elem-field">
		...
		<table class="posttable">
			...
			<tr>
			    ...
			    <td>
			        ...
			        尿潜血<input type="text" reg='{"maxlength":"50"}' name="eryQuantitativeValue" id="eryQuantitativeValue" value="${examination.eryQuantitativeValue}"  style="width: 80px;"/>
			   		&nbsp;&nbsp;
			        其他<input type="text" reg='{"maxlength":"100"}' name="urineRoutinesOtherDesc" id="urineRoutinesOtherDesc" value="${examination.urineRoutinesOtherDesc}"  style="width: 150px;"/>
			    </td>
			</tr>
			<tr>
			...
			<%--<tr>
			    <th>B超*</th>
			    <td>
			    	<label><input type="radio" onclick="util.clickHideText(this,'bmodeAnomalyfDesc')" name="bmodeAnomalyfFlag" value="2" ${examination.bmodeAnomalyfFlag eq "2" ? "checked" : ""}/>未检</label>
			        <label><input type="radio" onclick="util.clickHideText(this,'bmodeAnomalyfDesc')" name="bmodeAnomalyfFlag" ${examination.bmodeAnomalyfFlag eq "0" ?"checked":""} value="0"> 正常</label>
			        <label><input type="radio" id="bmodeAnomalyfFlag" onclick="util.clickShowText(this,'bmodeAnomalyfDesc')" name="bmodeAnomalyfFlag" ${examination.bmodeAnomalyfFlag eq"1" ?"checked":""} value="1"> 异常</label>
			    	<input type="text" id="bmodeAnomalyfDesc" name="bmodeAnomalyfDesc" title="${examination.bmodeAnomalyfDesc}" value="${examination.bmodeAnomalyfDesc}" CLASS="hidediv" style="width: 850px;"  reg='{"dependOn":"bmodeAnomalyfFlag","required":"true","maxlength":"100"}'>
			    </td>
			</tr>--%>
            <tr>
                <th class="required">B超</th>
                <td id="BUltrason">
                    腹部B超
                    <label><input type="radio" onclick="util.clickHideText(this,'bmodeAnomalyfDesc')" name="bmodeAnomalyfFlag" value="2" ${examination.bmodeAnomalyfFlag eq "2" ? "checked" : ""}/>未检</label>
                    <label><input type="radio" onclick="util.clickHideText(this,'bmodeAnomalyfDesc')" name="bmodeAnomalyfFlag" ${examination.bmodeAnomalyfFlag eq "0" ?"checked":""} value="0"> 正常</label>
                    <label><input type="radio" id="bmodeAnomalyfFlag" onclick="util.clickShowText(this,'bmodeAnomalyfDesc')" name="bmodeAnomalyfFlag" ${examination.bmodeAnomalyfFlag eq"1" ?"checked":""} value="1"> 异常</label>
                    <input type="text" id="bmodeAnomalyfDesc" name="bmodeAnomalyfDesc" title="${examination.bmodeAnomalyfDesc}" value="${examination.bmodeAnomalyfDesc}" CLASS="hidediv" style="width: 650px;"  reg='{"dependOn":"bmodeAnomalyfFlag","required":"true","maxlength":"100"}'>
                    <br>
                    其他B超
                    <label><input  type="radio"  onclick="util.clickHideText(this,'bmodeOtherAnomalyfDesc')" name="bmodeOtherAnomalyfFlag" value="2" ${examination.bmodeOtherAnomalyfFlag eq "2" ? "checked" : ""}/>未检</label>
                    <label><input  type="radio" onclick="util.clickHideText(this,'bmodeOtherAnomalyfDesc')" name="bmodeOtherAnomalyfFlag" ${examination.bmodeOtherAnomalyfFlag eq "0" ?"checked":""} value="0"> 正常</label>
                    <label><input  type="radio" onclick="util.clickShowText(this,'bmodeOtherAnomalyfDesc')" id="bmodeOtherAnomalyfFlag" name="bmodeOtherAnomalyfFlag" ${examination.bmodeOtherAnomalyfFlag eq"1" ?"checked":""} value="1"> 异常</label>
                    <input type="text" id="bmodeOtherAnomalyfDesc" name="bmodeOtherAnomalyfDesc" value="${examination.bmodeOtherAnomalyfDesc}" title="${examination.bmodeOtherAnomalyfDesc}" CLASS="hidediv" style="width: 650px;"  reg='{"dependOn":"bmodeOtherAnomalyfFlag","required":"true","maxlength":"100"}'>
                </td>
            </tr>
			<c:if test="${isWoman}">
				<tr>
				...
				<td>
		        	...
		        	<table id="drugHistoryDiv" class="tt_hidden">
		        		...
		        		<tr>
		        			...
		        			<th style="width: 11%">服药依从性</th>
		        			<td style="width: 49%"><ehr:dic-radio name="PersonalPhyExamDTO.drugHistoryList[1].medicationCompliance" dicmeta="FS10141" value="${drugHistoryList[0].medicationCompliance}"/></td>
		        		</tr>
		        		<tr>
	...
	<fieldset class="layui-elem-field">
		...
		<table class="posttable" id="CMedicine">
			...
			<tr>
			    ...
			    <td>
                        <ehr:dic-radio name="tcmPeacefulQuality" dicmeta="FS10186" code="1,2,0" value="${examination.tcmPeacefulQuality}"></ehr:dic-radio>
			    </td>
				<c:if test="${not empty echIdentification.peacefulQualityGuidance}">
					<td style="text-align: right">中医健康指导：</td>
					<td style="text-align: left">
						<ehr:dic  dicmeta="CHG10510" code="${echIdentification.peacefulQualityGuidance}"></ehr:dic>
						<c:forEach   items="${echIdentification.peacefulQualityGuidance}" var="ex">
							<c:if  test="${ex == '6'}">
								：${echIdentification.peacefulQualityGuidancestr}
							</c:if>
						</c:forEach>
					</td>
				</c:if>
				<c:if test="${empty echIdentification.peacefulQualityGuidance}">
					<td></td>
					<td></td>
				</c:if>
			</tr>
			<tr>
			    <th>气虚质</th>
			    <td>
                        <ehr:dic-radio name="tcmQiQuality" dicmeta="FS10186" code="1,3,0" value="${examination.tcmQiQuality}"></ehr:dic-radio>
			    </td>
              <c:if test="${not empty echIdentification.qiQualityGuidance}">
				<td style="text-align: right">中医健康指导：</td>
				<td style="text-align: left">
					<ehr:dic  dicmeta="CHG10510" code="${echIdentification.qiQualityGuidance}"></ehr:dic>
					<c:forEach   items="${echIdentification.qiQualityGuidance}" var="ex">
						<c:if  test="${ex == '6'}">
							：${echIdentification.qiQualityGuidancestr}
						</c:if>
					</c:forEach>
				</td>
           </c:if>
			...
			<tr>
			    <th>阳虚质</th>
			    <td>
                    <ehr:dic-radio name="tcmYangQuality" dicmeta="FS10186" code="1,3,0" value="${examination.tcmYangQuality}"></ehr:dic-radio>
			    </td>
            <c:if test="${not empty echIdentification.yangQualityGuidance}">
				<td style="text-align: right">中医健康指导：</td>
				<td style="text-align: left">
					<ehr:dic  dicmeta="CHG10510" code="${echIdentification.yangQualityGuidance}"></ehr:dic>
					<c:forEach   items="${echIdentification.yangQualityGuidance}" var="ex">
						<c:if  test="${ex == '6'}">
							：${echIdentification.yangQualityGuidancestr}
						</c:if>
					</c:forEach>
				</td>
           </c:if>
			...
			<tr>
			    <th>阴虚质</th>
			    <td>
                        <ehr:dic-radio name="tcmYinDeficiency" dicmeta="FS10186" code="1,3,0" value="${examination.tcmYinDeficiency}"></ehr:dic-radio>
			    </td>
              <c:if test="${not empty echIdentification.yinDeficiencyGuidance}">
				<td style="text-align: right">中医健康指导：</td>
				<td style="text-align: left">
					<ehr:dic  dicmeta="CHG10510" code="${echIdentification.yinDeficiencyGuidance}"></ehr:dic>
					<c:forEach   items="${echIdentification.yinDeficiencyGuidance}" var="ex">
						<c:if  test="${ex == '6'}">
							：${echIdentification.yinDeficiencyGuidancestr}
						</c:if>
					</c:forEach>
				</td>
            </c:if>
			...
			<tr>
			    <th>痰湿质</th>
			    <td>
                        <ehr:dic-radio name="tcmPhlegmWetness" dicmeta="FS10186" code="1,3,0" value="${examination.tcmPhlegmWetness}"></ehr:dic-radio>
			    </td>
                <c:if test="${not empty echIdentification.phlegmWetnessGuidance}">
				<td style="text-align: right">中医健康指导：</td>
				<td style="text-align: left">
					<ehr:dic  dicmeta="CHG10510" code="${echIdentification.phlegmWetnessGuidance}"></ehr:dic>
					<c:forEach   items="${echIdentification.phlegmWetnessGuidance}" var="ex">
						<c:if  test="${ex == '6'}">
							：${echIdentification.phlegmWetnessGuidancestr}
						</c:if>
					</c:forEach>
				</td>
            </c:if>
			...
			<tr>
			    <th>湿热质</th>
			    <td>
                        <ehr:dic-radio name="tcmHeatMedium" dicmeta="FS10186" code="1,3,0" value="${examination.tcmHeatMedium}"></ehr:dic-radio>
			     </td>
               <c:if test="${not empty echIdentification.heatMediumGuidance}">
				<td style="text-align: right">中医健康指导：</td>
				<td style="text-align: left">
					<ehr:dic  dicmeta="CHG10510" code="${echIdentification.heatMediumGuidance}"></ehr:dic>
					<c:forEach   items="${echIdentification.heatMediumGuidance}" var="ex">
						<c:if  test="${ex == '6'}">
							：${echIdentification.heatMediumGuidancestr}
						</c:if>
					</c:forEach>
				</td>
              </c:if>
			...
			<tr>
			    <th>血瘀质</th>
			    <td>
                        <ehr:dic-radio name="tcmBloodQuality" dicmeta="FS10186" code="1,3,0" value="${examination.tcmBloodQuality}"></ehr:dic-radio>
                </td>
              <c:if test="${not empty echIdentification.bloodQualityGuidance}">
				<td style="text-align: right">中医健康指导：</td>
				<td style="text-align: left">
					<ehr:dic  dicmeta="CHG10510" code="${echIdentification.bloodQualityGuidance}"></ehr:dic>
					<c:forEach   items="${echIdentification.bloodQualityGuidance}" var="ex">
						<c:if  test="${ex == '6'}">
							：${echIdentification.bloodQualityGuidancestr}
						</c:if>
					</c:forEach>
				</td>
               </c:if>
			...
			<tr>
			    <th>气郁质</th>
			    <td>
                        <ehr:dic-radio name="tcmQiStagnation" dicmeta="FS10186" code="1,3,0" value="${examination.tcmQiStagnation}"></ehr:dic-radio>
                </td>
               <c:if test="${not empty echIdentification.qiStagnationGuidance}">
				<td style="text-align: right">中医健康指导：</td>
				<td style="text-align: left">
					<ehr:dic  dicmeta="CHG10510" code="${echIdentification.qiStagnationGuidance}"></ehr:dic>
					<c:forEach   items="${echIdentification.qiStagnationGuidance}" var="ex">
						<c:if  test="${ex == '6'}">
							：${echIdentification.qiStagnationGuidancestr}
						</c:if>
					</c:forEach>
				</td>
               </c:if>
			...
			<tr>
			    <th>特秉质</th>
			    <td>
                        <ehr:dic-radio name="tcmSpecialQuality" dicmeta="FS10186" code="1,3,0" value="${examination.tcmSpecialQuality}"></ehr:dic-radio>
                </td>
                <c:if test="${not empty echIdentification.specialQualityGuidance}">
				<td style="text-align: right">中医健康指导：</td>
				<td style="text-align: left">
					<ehr:dic  dicmeta="CHG10510" code="${echIdentification.specialQualityGuidance}"></ehr:dic>
					<c:forEach   items="${echIdentification.specialQualityGuidance}" var="ex">
						<c:if  test="${ex == '6'}">
							：${echIdentification.specialQualityGuidancestr}
						</c:if>
					</c:forEach>
				</td>
              </c:if>
...
<%-- <div class="postdiv"  id="hypertensionLevelTr">
	<fieldset >
		<table class="posttable">
			<colgroup>
				<col style="width:15%">
			...
			<tr >
				<th style="width: 16%">血压水平分级</th>
				<td><ehr:dic-radio dicmeta="FS10283" value="${examination.hypertensionLevel}" id="hypertensionLevel" name="hypertensionLevel" /></td>
			</tr>
		</table>
	</fieldset>
</div>
<div class="postdiv">
	<fieldset>
		<table class="posttable">
			...
			<tr>
				<th style="width: 16%">危险因素和靶器官损害</th>
				<td><ehr:dic-radio dicmeta="FS10285" value="${examination.riskAndCriorganDamage}" id="riskAndCriorganDamage" name="riskAndCriorganDamage" /></td>
			</tr>
		</table>
	</fieldset>
</div>
<div class="postdiv" id="bloodGluAssessmentTr">
	<fieldset >
		<table class="posttable">
			<colgroup>
				<col style="width:15%">
			...
			<tr>
				<th style="width: 16%">血糖控制评估</th>
				<td><ehr:dic-radio dicmeta="FS10282" value="${examination.bloodGluAssessment}" id="bloodGluAssessment" name="bloodGluAssessment" /></td>
			</tr>
		</table>
	</fieldset>
</div>
<div class="postdiv">
	<fieldset>
		<table class="posttable">
			...
			<tr>
				<th style="width: 16%">综合评估</th>
				<td><ehr:dic-radio dicmeta="FS10284" value="${examination.overallAssessment}" id="overallAssessment" name="overallAssessment" /></td>
			</tr>
		</table>
	</fieldset>
</div> --%>
...
<%-- 					</ehr:authorize> --%>
			...
			<tr>
				...
				<td>
						<label><input type="radio" name="eatingAssessment" onclick="hmManageReport.calculateAssessment()" value="0"
							${examination.eatingAssessment eq 0 ? "checked" : "" }  reg="{'required':true}"/> 独立完成（0）</label>
						<label><input type="radio" name="eatingAssessment" onclick="hmManageReport.calculateAssessment()" value="3"
							${examination.eatingAssessment eq 3 ? "checked" : ""}  reg="{'required':true}"/> 需要协助（3）</label>
						<label><input type="radio" name="eatingAssessment" onclick="hmManageReport.calculateAssessment()" value="5"
			...
			<tr>
				...
				<td>
						<label><input type="radio" name="cleaningAssessment" onclick="hmManageReport.calculateAssessment()" value="0"
							${examination.cleaningAssessment eq 0 ? "checked" : "" } reg="{'required':true}"/> 独立完成（0）</label>
						<label><input type="radio" name="cleaningAssessment" onclick="hmManageReport.calculateAssessment()" value="1"
							${examination.cleaningAssessment eq 1 ? "checked" : ""} reg="{'required':true}"/> 洗澡需协助（1）</label>
						<label><input type="radio" name="cleaningAssessment" onclick="hmManageReport.calculateAssessment()" value="3"
							${examination.cleaningAssessment eq 3 ? "checked" : ""} reg="{'required':true}"/> 协助下能完成（3）</label>
						<label><input type="radio" name="cleaningAssessment" onclick="hmManageReport.calculateAssessment()" value="7"
			...
			<tr>
				...
				<td>
						<label><input type="radio" name="clothingAssessment" onclick="hmManageReport.calculateAssessment()" value="0"
							${examination.clothingAssessment eq 0 ? "checked" : ""} reg="{'required':true}"/> 独立完成（0）</label>
						<label><input type="radio" name="clothingAssessment" onclick="hmManageReport.calculateAssessment()" value="3"
							${examination.clothingAssessment eq 3 ? "checked" : ""} reg="{'required':true}"/> 需要协助（3）</label>
						<label><input type="radio" name="clothingAssessment" onclick="hmManageReport.calculateAssessment()" value="5"
			...
			<tr>
				...
				<td>
						<label><input type="radio" name="defecationAssessment" onclick="hmManageReport.calculateAssessment()" value="0"
							${examination.defecationAssessment eq 0 ? "checked" : ""} reg="{'required':true}"/> 不需协助（0）</label>
						<label><input type="radio" name="defecationAssessment" onclick="hmManageReport.calculateAssessment()" value="1"
							${examination.defecationAssessment eq 1 ? "checked" : ""} reg="{'required':true}"/> 偶尔失禁（1）</label>
						<label><input type="radio" name="defecationAssessment" onclick="hmManageReport.calculateAssessment()" value="5"
							${examination.defecationAssessment eq 5 ? "checked" : ""} reg="{'required':true}"/> 经常失禁（5）</label>
						<label><input type="radio" name="defecationAssessment" onclick="hmManageReport.calculateAssessment()" value="10"
							${examination.defecationAssessment eq 10 ? "checked" : ""} reg="{'required':true}"/> 完全失禁（10）</label>
				</td>
			</tr>
			<tr>
				<th><label class="required">活动：站立、室内行走、上下楼梯、户外活动</label></th>
				<td>
						<label><input type="radio" name="exerciseAssessment" onclick="hmManageReport.calculateAssessment()" value="0"
							${examination.exerciseAssessment eq 0 ? "checked" : ""} reg="{'required':true}"/> 独立完成（0）</label>
						<label><input type="radio" name="exerciseAssessment" onclick="hmManageReport.calculateAssessment()" value="1"
							${examination.exerciseAssessment eq 1 ? "checked" : ""} reg="{'required':true}"/> 借助较小外力（1）</label>
						<label><input type="radio" name="exerciseAssessment" onclick="hmManageReport.calculateAssessment()" value="5"
							${examination.exerciseAssessment eq 5 ? "checked" : ""} reg="{'required':true}"/> 借助较大外力（5）</label>
						<label><input type="radio" name="exerciseAssessment" onclick="hmManageReport.calculateAssessment()" value="10"
			...
			<tr>
				...
				<td>
						<label><input type="radio" name="cognitionScreenResult"
									  ...
									  value="1" reg="{'required':true}"/></label>
						粗筛阴性</label>
			...
			<tr>
				...
				<td>
						...
						<label><input type="radio" id="emotionScreenResult" name="emotionScreenResult"
									  ...
									  value="0" reg="{'required':true}"/></label>
						粗筛阳性
					<span id="depressionScore" ${examination.emotionScreenResult ne'0' ?'class="hidediv"' :''}>
						，老年人抑郁评分检查，总分<tag:numberInput name="depressionScore" value="${examination.depressionScore}"
			...
			<table class="posttable">
				...
				<tr>
					...
					<td>
						...
						<br>
						<label>
							<input type="checkbox" name="riskDiet" ${examination.riskDiet eq '1' ? 'checked' : ''}
								   value="1"/>
							<span>饮食</span>
						</label>
						<br>
						<label>
							<input type="checkbox" name="riskExercise" ${examination.riskExercise eq '1' ? 'checked' : ''}
								   value="1"/>
							<span>锻炼</span>
						</label>
						<br>
						<label>
							<input type="checkbox" id="riskWeightReduction" onclick="util.clickShowText(this,'riskWeightReductionSpan')"
					...
					<span id="riskWeightReductionSpan" class="hidediv">目标:&nbsp;
						 ...
						 <tag:numberInput point="point" value="${examination.riskWeightTarget}" name="riskWeightTarget"  style="width: 200px;" 
										  cssClass="width30 "
										  reg="{'dependOn':'riskWeightReduction','required':'true','min':0,'max':9999}"/>Kg
					</span>
						<br>
						<label>
							<input type="checkbox" id="guideVaccination" onclick="util.clickShowText(this,'guideVaccinationSpan')"
								   name="guideVaccination" ${examination.guideVaccination eq '1' ? 'checked' : ''}
								   value="1"/>
							<span>建议接种疫苗</span>
						</label>
					<span id="guideVaccinationSpan" class="hidediv"> <input type="text" name="guideVaccinationDesc"  style="width: 200px;" 

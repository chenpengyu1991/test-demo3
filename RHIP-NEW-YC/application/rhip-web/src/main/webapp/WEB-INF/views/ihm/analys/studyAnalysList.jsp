...
<div id="cpwlisttableDiv" <%-- <c:if test="${genreCode != '0' }">class="contentfixed144"</c:if> style="min-width: 1000px; width: 100%;overflow-x: auto; margin-left: 0px;" --%>>
    <table style="width: 100%;" style="width: 100%;" class="layui-table x-admin-sm-table-list-middle">
        <colgroup>
            <c:choose>
                <c:when test="${genreCode == '0' }">
                    <col style="width: 160px;"/>
        ...
        <c:forEach var="report" items="${summaryList}" varStatus="status">
            <tr>
                <c:choose>
	              	...
	              	<c:when test="${genreCode == HOSPITAL || genreCode == CENTRE}">
	              		<c:choose>
	              			...
	              			<c:when test="${report.gbCode == '小计' || report.organ_code == '小计'}">
                ...
                <td class="centertd"><tags:numberLabel value="${report.BUSNUM}" defaultValue="0" align="center"/></td>
                <td class="centertd"><tags:numberLabel value="${report.CXRNUM}" defaultValue="0" align="center"/></td>
                <td class="centertd"><tags:numberLabel value="${report.ECGNUM}" defaultValue="0" align="center"/></td>
                <td class="centertd"><tags:numberLabel value="${report.CTNUM}" defaultValue="0" align="center"/></td>
            </tr>
        </c:forEach>
        </tbody>
    </table>

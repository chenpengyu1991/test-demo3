...
<div>
		...
		<div>
			<table class="formtable" id="popEsTable">
				<tr>
					...
					<th style="width: 15%"><label class="required">患者姓名</label></th>
    ...
    <div class="toolbarpop">
        <c:if test="${type == 'add'}">
            <input type="button" id="saveContactPerson" value="添加" onclick="varicellaCase.addEsList()">
        </c:if>
        <c:if test="${type == 'edit'}">
            <input type="button" id="editContactPerson" value="修改" onclick="varicellaCase.editEsList()">
        </c:if>

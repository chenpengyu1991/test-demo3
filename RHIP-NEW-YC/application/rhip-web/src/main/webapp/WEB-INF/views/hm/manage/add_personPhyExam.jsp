...
$(function() {
		/*var physicalExamCode = $("input[name='physicalExamCode']").val();
		if(physicalExamCode!= null && physicalExamCode!=""){
			$("[name='manaDoctorId']").attr("editable",false);
		}*/
...
<script src="${pageContext.request.contextPath}/js/views/ehr/person/phyExamUtil.js" type="text/javascript"></script>	
  <script src="${pageContext.request.contextPath}/js/views/ehr/person/phyExam.js" type="text/javascript" ></script>
	...
	<tr>
		...
		<th colspan="5"><!-- <label class="required">体检年份</label> --></th>
		<td>
			<%-- <c:choose>
				<c:when test="${not empty examination.examYear}">
					<tag:dateInput name="examYear" date="${examination.examYearDate}" pattern="yyyy" reg="{'required':true}" style="width:120px;margin-bottom: 5px;"/>
				</c:when>
				<c:when test="${not empty examination.examinationDate}">
					<tag:dateInput name="examYear" date="${examination.examinationDate}" pattern="yyyy" reg="{'required':true}" style="width:120px;margin-bottom: 5px;"/>
				</c:when>
				<c:otherwise>
					<tag:dateInput name="examYear" date="${today}" pattern="yyyy" reg="{'required':true}" style="width:120px;margin-bottom: 5px;"/>
				</c:otherwise>
			</c:choose> --%>
		</td>					
		<th >编号 </th>
		<td>
			<c:choose>
				<c:when test="${not empty examination.physicalExamCode}">
				 	<input style="width:130px;margin-bottom: 5px;" type="text" name='physicalExamCode' value="${examination.physicalExamCode}" disabled="disabled"/>
				</c:when>
				<c:otherwise>
				 	<input style="width:130px;margin-bottom: 5px;" type="text" value="自动生成" disabled="disabled"/>
				</c:otherwise>
			</c:choose>
		</td>
	</tr>
	<tr>
			...
			<td>
					<lable class="text_tip_span"><tag:autoSelect reg="{'required':true}"  name="examinationOrganCode" id="examinationOrganCode" codeValue="${examination.examinationOrganCode}"></tag:autoSelect></lable>
			</td>
			<th><label class="required">体检日期</label></th>
			<td>
				<c:if test="${empty examination.examinationDate}">
					<input type="text" class="layui-input x-admin-content-sm-date" reg="{'required':true}"  name="examinationDate" id="examinationDate_input" value="<fmt:formatDate value='${today}' pattern='yyyy/MM/dd'/>" style="padding-left: 0px;width: 178px;margin-bottom: 5px;" />
				</c:if> 
				<c:if test="${not empty examination.examinationDate}">
					<input type="text" class="layui-input x-admin-content-sm-date" reg="{'required':true}"  name="examinationDate" id="examinationDate_input" value="<fmt:formatDate value='${examination.examinationDate}' pattern='yyyy/MM/dd'/>" style="padding-left: 0px;width: 178px;margin-bottom: 5px;" />
				</c:if>
			</td>
			<th>责任医生</th>
			...
			<ehr:authorize ifNotGranted="01">
				<td><ehr:staff-list name="manaDoctorId"
									value="${examination.manaDoctorId}" style="width:130px;margin-bottom: 5px;" defaultval="Y" orgCode=""
						></ehr:staff-list>
				</td>
			</ehr:authorize>
		</tr>
	...
	<fieldset class="layui-elem-field">
		...
		<table class="posttable">
			<tr>
			    ...
			    <td >
			    	<tag:numberInput point="point" style="width: 80px" value="${examination.temperature}" id="temperature" name="temperature"  reg="{'min':0,'max':999.9}"></tag:numberInput>℃</td>
			    <th>脉率</th>
			    <td style="width: 42%"><tag:numberInput style="width: 80px" id="pulseRate" name="pulseRate" value="${examination.pulseRate}" reg="{'min':0,'max':9999}"/>次/分钟</td>
			</tr>
			<tr>
			    <th>呼吸频率</th>
			    <td><tag:numberInput name="respiratoryRate" id="respiratoryRate" style="width: 80px"  value="${examination.respiratoryRate}" reg="{'min':0,'max':9999}"/>次/分钟</td>
			    <th>血压</th>
			    <td>
			        左侧
			                  ...
			                  右侧
			                        <tag:numberInput  name="rightSbp" value="${examination.rightSbp}" style="width: 40px" reg="{'min':0,'max':9999}" id="rightBloodUp"/>/
			                        <tag:numberInput  name="rightDbp" value="${examination.rightDbp}"  style="width: 40px"  reg="{'min':0,'max':9999}" id="rightBloodDown"/>mmHg
			...
			<th><label class="required">来源</label></th>
				<td>
					<ehr:dic-radio dicmeta="TFJKDA001" name="bloodPressureSource" value="${examination.bloodPressureSource}" reg="{'required':true}"/>
				</td>
			</tr>
		...
		<fieldset class="layui-elem-field">
			...
			<table class="posttable" >
				<tr>
					<th width="16%">锻炼频率</th>
				    <td >
				        <ehr:dic-radio name="trainFrequencyTypeCode" dicmeta="FS10208" value="${examination.trainFrequencyTypeCode}"  onchange="cdmPersonPhyExamAdd.setContentStyleSport(this,'exerciseTypeSpan:keepTimeSpan:exerciseTimeSpan')"/>
				    </td>
				</tr>
				<tr>
				    <th>每次锻炼时间</th>
				    ...
				    <td>
				    	<span id="keepTimeSpan">
				        	<tag:numberInput reg="{'min':0,'max':99999.9}" point="point" name="trainingTotaltime" value="${examination.trainingTotaltime}" style="width: 80px" />年
				        </span>
				    </td>
				</tr>
				<tr>
				    ...
				    <span id="exerciseTypeSpan">
				    	...
				    	<span id="other_train_way_span" class="hidediv">
				    		<input type="text" reg='{"extension":["otherTrainWayVali","不能为空"],"maxlength":"100"}' name="otherTrainingWay" value="${examination.otherTrainingWay}" style="width: 150px"/>
				    	</span>
				    </span>
					</td>
				</tr>
		...
		<fieldset class="layui-elem-field">
			...
			<colgroup>
				...
				<tr>
					<th width="16%">吸烟状况</th>
				    <td >
				       <ehr:dic-radio name="smodeStatusCode" dicmeta="CV0300101" value="${examination.smodeStatusCode}" onchange="cdmPersonPhyExamAdd.setContentStyle(this,'smokePerDaySpan:beginSmokeAgeSpan:endSmokeAgeSpan')"/>
				   </td>
				</tr>
				<tr>
				    <th>日吸烟量</th>
		...
		<fieldset class="layui-elem-field">
			...
			<colgroup>
				...
				<tr>
					<th width="16%">饮酒频率</th>
				    <td >
				    	<ehr:dic-radio code="1,2,3,4" name="drinkFrequency" value="${examination.drinkFrequency }" dicmeta="CV0300104"  onchange="cdmPersonPhyExamAdd.setContentStyle(this,'wineTypeSpan:isDrunkSpan:beginDrinkingAgeSpan:isEndDrinkingSpan:drinkingPerDaySpan')"/>
				  	</td>
				</tr>
				<tr>
				    <th>日饮酒量</th>
					...
					<td>
						...
						<label>
			             ...
			             <div  class="tt_hidden" id="ttb2">
			                    &nbsp;(工种<input reg='{"dependOn":"occupationExposureFlag","required":true,"maxlength":"33"}' type="text" name="riskOccupationDesc" value="${examination.riskOccupationDesc}"  style="width: 80px"/> 从业时间
			...
			<tr>
				...
				<td style="width: 84%">
					<table>
						...
						<tr>
			                <td colspan="4">齿列&nbsp;
			                  <input type="radio" name="dentitionAnomalyFlag" onclick="util.clickHideTable(this,'ttb7')" ${examination.dentitionAnomalyFlag eq '0' or examination.dentitionAnomalyFlag eq null ? "checked" : ""} value="0">正常
			                  <input type="radio" id="dentitionAnomalyFlag" name="dentitionAnomalyFlag" onclick="util.clickShowTable(this,'ttb7')" ${examination.dentitionAnomalyFlag eq '1' ?"checked" :""}  value="1">异常
	...
	<fieldset class="layui-elem-field">
		...
		<table class="posttable">
			...
			<tr>
			    ...
			    <td  style="width: 84%;">
					<input type="radio" id="fundusOculiAnomalyFlagWJ" onclick="util.clickHideText(this,'fundusOculiAnomalyDesc')" name="fundusOculiAnomalyFlag" ${examination.fundusOculiAnomalyFlag eq "2" ? "checked" : ""} value="2">未检
			        <input type="radio" id="fundusOculiAnomalyFlagZC" onclick="util.clickHideText(this,'fundusOculiAnomalyDesc')" name="fundusOculiAnomalyFlag" ${examination.fundusOculiAnomalyFlag eq "0" or examination.fundusOculiAnomalyFlag eq null ? "checked" : ""} value="0">正常
			...
			<tr>
			    <th>皮肤</th>
			    <td >
			        <input type="radio" id="skinZC" onclick="util.clickHideText(this,'skinCheckDesc')" name="skinCheckResult" ${examination.skinCheckResult eq "1" or examination.skinCheckResult eq null ? "checked" : ""} value="1">正常
			        <input type="radio" id="skinCH" onclick="util.clickHideText(this,'skinCheckDesc')"  name="skinCheckResult" ${examination.skinCheckResult eq"2" ?"checked":""} value="2">潮红
			        <input type="radio" id="skinCB" onclick="util.clickHideText(this,'skinCheckDesc')"  name="skinCheckResult" ${examination.skinCheckResult eq"3" ?"checked":""} value="3">苍白
			        <input type="radio" id="skinFQ" onclick="util.clickHideText(this,'skinCheckDesc')"  name="skinCheckResult" ${examination.skinCheckResult eq"4" ?"checked":""} value="4">发绀
			        <input type="radio" id="skinHR" onclick="util.clickHideText(this,'skinCheckDesc')"  name="skinCheckResult" ${examination.skinCheckResult eq"5" ?"checked":""} value="5">黄染
			        <input type="radio" id="skinSS" onclick="util.clickHideText(this,'skinCheckDesc')"  name="skinCheckResult" ${examination.skinCheckResult eq"6" ?"checked":""} value="6">色素沉着
			...
			<tr>
			    <th>巩膜</th>
			    <td >
			        <input type="radio"  id="scleraZC" onclick="util.clickHideText(this,'scleraCheckDesc')"   name="scleraCheckResult" ${examination.scleraCheckResult eq "1" or examination.scleraCheckResult eq null ? "checked" : ""} value="1">正常
			        <input type="radio"  id="scleraHR" onclick="util.clickHideText(this,'scleraCheckDesc')"   name="scleraCheckResult" ${examination.scleraCheckResult eq"2" ?"checked":""} value="2">黄染
			        <input type="radio"  id="scleraBlood" onclick="util.clickHideText(this,'scleraCheckDesc')"   name="scleraCheckResult" ${examination.scleraCheckResult eq"3" ?"checked":""} value="3">充血
			...
			<tr>
			    <th>淋巴结</th>
			    <td >
			        <input type="radio" id="lymphNodeZC" onclick="util.clickHideText(this,'lymphNodeCheckDesc')"  name="lymphNodeCheckResult" ${examination.lymphNodeCheckResult eq "1" or examination.lymphNodeCheckResult eq null ?"checked":""} value="1">未触及
			        <input type="radio" id="lymphNodeSG" onclick="util.clickHideText(this,'lymphNodeCheckDesc')"  name="lymphNodeCheckResult" ${examination.lymphNodeCheckResult eq"2" ?"checked":""} value="2">锁骨上
			        <input type="radio" id="lymphNodeWY" onclick="util.clickHideText(this,'lymphNodeCheckDesc')"  name="lymphNodeCheckResult" ${examination.lymphNodeCheckResult eq"3" ?"checked":""} value="3">腋窝
					...
					<table>
						<tr>
							<td style="width: 100%;">桶状胸:
								<input id="barrelChestZC" type="radio" name="barrelChest" ${examination.barrelChest eq "0" or examination.barrelChest eq null ? "checked" : ""} value="0">否
						        <input id="barrelChest" type="radio" name="barrelChest" ${examination.barrelChest eq "1" ?"checked":""} value="1">是
					        </td>
				        </tr>
				        <tr>
						    <td>呼吸音:
						        <input type="radio" id="lungsAnomalySoundZC" onclick="util.clickHideText(this, 'lungsAnomalyDesc')" name="lungsAnomalySound" ${examination.lungsAnomalySound eq "1" or examination.lungsAnomalySound eq null ?"checked":""} value="1">正常
						...
						<tr>
						    <td colspan="4">啰&nbsp;&nbsp;&nbsp;音:
						        <input type="radio" id="lungsRaleZC" onclick="util.clickHideText(this, 'lungsRaleDesc')" name="lungsRaleFlag" ${examination.lungsRaleFlag eq "1" or examination.lungsRaleFlag eq null  ?"checked":""} value="1">无
						        <input type="radio" id="lungsRaleFlagG" onclick="util.clickHideText(this, 'lungsRaleDesc')"  name="lungsRaleFlag" ${examination.lungsRaleFlag eq"2" ?"checked":""} value="2">干啰音
						        <input type="radio" id="lungsRaleFlagS" onclick="util.clickHideText(this, 'lungsRaleDesc')"  name="lungsRaleFlag" ${examination.lungsRaleFlag eq"3" ?"checked":""} value="3">湿啰音
			...
			<tr>
				...
				<td >
					<table>
						<tr>
							<td>
							          心率:<tag:numberInput reg="{'min':0,'max':999}" id="heartRate" name="heartRate" value="${examination.heartRate}"  style="width: 40px;" />次/分钟
							</td>
				        </tr>
				        <tr>
				        	<td>
				        		心律: <ehr:dic-radio id="cardioverter" name="cardioverter" dicmeta="FS10176" value="${examination.cardioverter}"/>
				        	</td>
				        </tr>
				        <tr>
				        	<td>
								杂音: <input type="radio" id="heartMurmurFlagZC" onclick="util.clickHideText(this,'heartMurmurDesc')" name="heartMurmurFlag" ${examination.heartMurmurFlag eq "0" or examination.heartMurmurFlag eq null  ? "checked" : ""} value="0">无
				...
				<td >
					<table>
						<tr>
						   <td style="width: 100%">
						              压痛:
				             <input type="radio" id="abdominalTendernessFlagZC" onclick="util.clickHideText(this,'abdominalTendernessDesc')" name="abdominalTendernessFlag" ${examination.abdominalTendernessFlag eq "0" or examination.abdominalTendernessFlag eq null ? "checked" : ""} value="0"/>无
						...
						<tr>
						    <td>包块:
						        <input type="radio" id="abdominalMassFlagZC" onclick="util.clickHideText(this,'abdominalMassDesc')" name="abdominalMassFlag" ${examination.abdominalMassFlag eq "0" or examination.abdominalMassFlag eq null ? "checked" : ""} value="0">无
						...
						<tr>
						    <td>肝大:
						        <input type="radio" id="liverFlagZC" onclick="util.clickHideText(this,'liverDesc')" name="liverFlag" ${examination.liverFlag eq "0" or examination.liverFlag eq null ? "checked" : ""} value="0">无
						...
						<tr>
						    <td>脾大:
						        <input type="radio" id="splenomegalyFlagZC" onclick="util.clickHideText(this,'splenomegalyDesc')" name="splenomegalyFlag" ${examination.splenomegalyFlag eq "0" or examination.splenomegalyFlag eq null ? "checked" : ""} value="0">无
						...
						<tr>
						    <td>移动性浊音:
						        <input type="radio" id="abdominalVoicedFlagZC" onclick="util.clickHideText(this,'abdominalVoicedDesc')" name="abdominalVoicedFlag" ${examination.abdominalVoicedFlag eq "0" or examination.abdominalVoicedFlag eq null ? "checked" : ""} value="0">无
			...
			<tr>
				<th>下肢水肿</th>
			    <td >
			        <ehr:dic-radio id="legsEdema" name="legsEdemaCheckResult" dicmeta="CV0410014"  value="${examination.legsEdemaCheckResult}"/>
			    </td>
			</tr>
			<tr>
			    <th>足背动脉搏动</th>
			    <td >
			        <ehr:dic-radio id="arteriopalmus" name="arteriopalmus" dicmeta="CV0410015" value="${examination.arteriopalmus}"/>
			    </td>
			</tr>
		<tr>
				<th>肛门指诊*</th>
				<td><label><input id="dreWJ" type="radio" name="dreCheckResultType" ${examination.dreCheckResultType eq "0" ? "checked" : ""}
						value="0" onchange="toggleOther('dreCheckResultType','dreCheckResultType_Span',9)" />未检</label>
					<ehr:dic-radio id="dre" name="dreCheckResultType" dicmeta="CV0410013"
						value="${examination.dreCheckResultType}" onchange="toggleOther('dreCheckResultType','dreCheckResultType_Span',9)" />
					<span style="display: none;" id="dreCheckResultType_Span"> 描述:<input reg='{"maxlength":"33"}' type="text"
						id="dreCheckResultDesc_inputId" name="dreCheckResultDesc" value="${examination.dreCheckResultDesc}" style="width: 200px;" />
				</span></td>
			</tr>
	...
	<fieldset>
		...
		<table class="posttable">
			...
			<tr>
			    <th>空腹血糖</th>
			    <td>
			       <tag:numberInput point="point" reg='{"max":"99.99","scale":"2","regex":"number"}' name="fpgMmol" value="${examination.fpgMmol}"  style="width: 40px;"/>mmol/L
			        或
			        <tag:numberInput point="point" reg='{"max":"99.99","scale":"2","regex":"number"}'  name="fpgMg" value="${examination.fpgMg}"  style="width: 40px;" />mg/dL
			    </td>
			</tr>
			<tr>
			    <th>心电图</th>
			    <td>
			        ...
			        <input type="text" id="ecgAnomalyDesc" name="ecgAnomalyDesc" value="${examination.ecgAnomalyDesc}" CLASS="hidediv" style="width: 100px;" reg='{"dependOn":"ecgAnomalyFlag","required":"true","maxlength":"33"}'>
			    </td>
			</tr>
			...
			<tr>
			    <th>肾功能</th>
			    <td>
			        血清肌酐 <tag:numberInput point="point" reg="{'min':0,'max':999.9}"  name="creatinine" value="${examination.creatinine}"  style="width: 80px;" />μmol/L
			        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	...
	<fieldset class="layui-elem-field">
		...
		<table class="posttable">
			<tr>
				...
				<td>
				          血红蛋白<tag:numberInput reg="{'min':0,'max':9999}" name="hemoglobinValue" id="hemoglobinValue" value="${examination.hemoglobinValue}" style="width: 40px;"/>g/L
				         &nbsp;&nbsp;&nbsp;&nbsp;
				          白细胞<tag:numberInput reg="{'min':0,'max':999.9}" point="point"  id="leukocyteCount" name="leukocyteCount" value="${examination.leukocyteCount}"  style="width: 40px;" /> ×10<sup>9</sup>/L
				        &nbsp;&nbsp;&nbsp;&nbsp;
				          血小板<tag:numberInput reg="{'min':0,'max':9999}" name="plateletCount" id="plateletCount" value="${examination.plateletCount}"  style="width: 40px;" /> ×10<sup>9</sup>/L
				    	&nbsp;&nbsp;&nbsp;&nbsp;      
			...
			<tr>
			    <th>尿常规*</th>
			    <td>
			        尿蛋白<input type="text" reg='{"maxlength":"50"}' name="urineProQuantitativeValue"id="urineProQuantitativeValue" value="${examination.urineProQuantitativeValue}"  style="width: 80px;"/>
			        &nbsp;&nbsp;
			        尿糖<input type="text" reg='{"maxlength":"50"}' name="urineSugQuantitativeValue"id="urineSugQuantitativeValue" value="${examination.urineSugQuantitativeValue}" style="width: 80px;"/>
			        &nbsp;&nbsp;
			        尿酮体<input type="text" reg='{"maxlength":"50"}'  name="ketQuantitativeValue" id="ketQuantitativeValue"value="${examination.ketQuantitativeValue}"  style="width: 80px;" />
			        &nbsp;&nbsp;
			        尿潜血<input type="text" reg='{"maxlength":"50"}' name="eryQuantitativeValue"id="eryQuantitativeValue" value="${examination.eryQuantitativeValue}"  style="width: 80px;"/>
			   		&nbsp;&nbsp;
			        其他<input type="text" reg='{"maxlength":"100"}' name="urineRoutinesOtherDesc" value="${examination.urineRoutinesOtherDesc}"  style="width: 150px;"/>
			    </td>
			</tr>
			<tr>
			    ...
			    <td id="ttb15">
			       <tag:numberInput showlen="1" point="point" reg='{"extension":["FastingBloodVali","请至少选择一项"],"max":"99.99","scale":"2","regex":"number"}' name="fpgMmol" value="${examination.fpgMmol}"  style="width: 40px;" id="bloodGlucoseLeft"/>mmol/L
			        或
			        <tag:numberInput showlen="1" point="point" reg='{"extension":["FastingBloodVali","请至少选择一项"],"max":"99.99","scale":"2","regex":"number"}'  name="fpgMg" value="${examination.fpgMg}"  style="width: 40px;"  id="bloodGlucoseRight"/>mg/dL
			    </td>
			</tr>
			...
			<label class="required">来源</label></th>
				<td>
					<ehr:dic-radio dicmeta="TFJKDA001" name="bloodSugarSource" value="${examination.bloodSugarSource}" reg="{'required':true}"/>
				</td>
			</tr>
			<tr>
			    <th>心电图*</th>
			    <td>
			    	<label><input type="radio"  name="ecgAnomalyFlag" ${examination.ecgAnomalyFlag eq "2" ? "checked" : ""} value="2"> 未检</label>
			        <label><input type="radio"  name="ecgAnomalyFlag" ${examination.ecgAnomalyFlag eq "0" ? "checked" : ""} value="0"> 正常</label>
			        <label><input type="radio" id="ecgAnomalyFlag"  name="ecgAnomalyFlag" ${examination.ecgAnomalyFlag eq"1" ?"checked":""} value="1"> 异常</label>
			        <input type="text" id="ecgAnomalyDesc" name="ecgAnomalyDesc" value="${examination.ecgAnomalyDesc}"  style="width: 700px;" reg='{"dependOn":"ecgAnomalyFlag","required":"true","maxlength":"50"}'>
			    </td>
			</tr>
			<tr>
			    <th>尿微量白蛋白*</th>
			    <td>
			        <tag:numberInput point="point" reg="{'min':0,'max':9999.9}" name="urineMicroTongAlbumin" id="urineMicroTongAlbumin" value="${examination.urineMicroTongAlbumin}"  style="width: 40px;"/>mg/dL
			    </td>
			</tr>
			<tr>
			    <th>大便潜血*</th>
			...
			<tr>
			    <th>糖化血红蛋白*</th>
			    <td>
			        <tag:numberInput  point="point" reg='{"max":"99.99","scale":"2","regex":"number"}' name="hgb" id="hgb" value="${examination.hgb}"  style="width: 40px;" />%
			    </td>
			</tr>
			<tr>
			    <th>乙型肝炎表面抗原*</th>
			...
			<tr>
			    <th>肝功能*</th>
			    <td>
			        血清谷丙转氨酶 <tag:numberInput point="point" reg="{'min':0,'max':999.99}"  name="serumGptValue" id="serumGptValue" value="${examination.serumGptValue}" style="width: 80px;" />U/L
			        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			        血清谷草转氨酶 <tag:numberInput point="point" reg="{'min':0,'max':999.99}"  name="serumAstValue" id="serumAstValue" value="${examination.serumAstValue}"  style="width: 80px;" />U/L
			        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			        白蛋白浓度  <tag:numberInput reg="{'min':0,'max':999.9}" point="point" name="albuminConcentration" id="albuminConcentration" value="${examination.albuminConcentration}"  style="width: 80px;"  />g/L
			        <br>
			        总胆红素  <tag:numberInput point="point" reg="{'min':0,'max':999.9}"  name="totalBilirubin" id="totalBilirubin" value="${examination.totalBilirubin}" style="width: 80px;" />μmol/L
			        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					...
					<input type="text"  name="conjugatedBilirubin" id="conjugatedBilirubin"value="${examination.conjugatedBilirubin}"  style="width: 80px;" />μmol/L
			    </td>
			</tr>
			<tr>
			    <th>肾功能*</th>
			    <td>
			        血清肌酐 <tag:numberInput point="point" reg="{'min':0,'max':999.9}"  name="creatinine" id="creatinine" value="${examination.creatinine}"  style="width: 80px;" />μmol/L
			        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			        血尿素氮 <tag:numberInput point="point" reg="{'min':0,'max':999.9}"  name="bloodUreaNitrogenValue" id="bloodUreaNitrogenValue"  value="${examination.bloodUreaNitrogenValue}" style="width: 80px;" />mmol/L
			        <br>
			        血钾浓度  <tag:numberInput point="point"  reg="{'min':0,'max':999.9}" name="potassiumConcentration" id="potassiumConcentration"  value="${examination.potassiumConcentration}"  style="width: 80px;" />mmol/L
			        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			        血钠浓度  <tag:numberInput  reg="{'min':0,'max':9999}"  name="sodiumConcentration" id="sodiumConcentration"  value="${examination.sodiumConcentration}" style="width: 80px;"  />mmol/L
			    </td>
			</tr>
			<tr>
			    <th>血脂*</th>
			    <td >
			        总胆固醇 <tag:numberInput point="point" reg="{'min':0,'max':999.99}" name="tc" id="tc" value="${examination.tc}" style="width: 80px;" />mmol/L
			        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			        甘油三酯 <tag:numberInput point="point" reg="{'min':0,'max':99.9}" name="triglycerideValue" id="triglycerideValue" value="${examination.triglycerideValue}"  style="width: 80px;"  />mmol/L
			        <br>
			        血清低密度脂蛋白胆固醇 <tag:numberInput reg="{'min':0,'max':999.99}" point="point" name="ldlcDetectValue" id="ldlcDetectValue" value="${examination.ldlcDetectValue}"  style="width: 80px;" />mmol/L
			        <br>
			        血清高密度脂蛋白胆固醇  <tag:numberInput reg="{'min':0,'max':999.99}" point="point" name="hdlcDetectValue" id="hdlcDetectValue" value="${examination.hdlcDetectValue}" style="width: 80px;" />mmol/L
			    </td>
			</tr>
			<tr>
			    <th>胸部X线片*</th>
			    <td>
			    	<label><input type="radio"  name="chestXAnomalyfFlag" value="2" ${examination.chestXAnomalyfFlag eq "2" ? "checked" : ""}/>未检</label>
			        <label><input type="radio"  name="chestXAnomalyfFlag" ${examination.chestXAnomalyfFlag eq "0" ?"checked":""} value="0"> 正常</label>
			        <label><input type="radio" id="chestXAnomalyfFlag"  name="chestXAnomalyfFlag" ${examination.chestXAnomalyfFlag eq"1" ?"checked":""} value="1"> 异常</label><br>
			        <input type="text" id="chestXAnomalyfDesc" name="chestXAnomalyfDesc" value="${examination.chestXAnomalyfDesc}"  style="width: 650px;"  reg='{"maxlength":"33"}'>
			    </td>
			</tr>
			<tr>
                <th>B超*</th>
			    <td id="BUltrason">
                    腹部B超：
			    	<label><input type="radio"  name="bmodeAnomalyfFlag" value="2" ${examination.bmodeAnomalyfFlag eq "2" ? "checked" : ""}/>未检</label>
			        <label><input type="radio"  name="bmodeAnomalyfFlag" ${examination.bmodeAnomalyfFlag eq "0" ?"checked":""} value="0"> 正常</label>
			        <label><input type="radio"  id="bmodeAnomalyfFlag"  name="bmodeAnomalyfFlag" ${examination.bmodeAnomalyfFlag eq"1" ?"checked":""} value="1"> 异常</label><br>
			        <input type="text" id="bmodeAnomalyfDesc" name="bmodeAnomalyfDesc" title="${examination.bmodeAnomalyfDesc}" value="${examination.bmodeAnomalyfDesc}"  style="width: 650px;"  reg='{"maxlength":"100"}'>
                    <br>
                    其他B超：
                    <label><input  type="radio"   name="bmodeOtherAnomalyfFlag" value="2" ${examination.bmodeOtherAnomalyfFlag eq "2" ? "checked" : ""}/>未检</label>
                    <label><input  type="radio"  name="bmodeOtherAnomalyfFlag" ${examination.bmodeOtherAnomalyfFlag eq "0" ?"checked":""} value="0"> 正常</label>
                    <label><input  type="radio"  id="bmodeOtherAnomalyfFlag" name="bmodeOtherAnomalyfFlag" ${examination.bmodeOtherAnomalyfFlag eq"1" ?"checked":""} value="1"> 异常</label><br>
                    <input type="text" id="bmodeOtherAnomalyfDesc" name="bmodeOtherAnomalyfDesc" value="${examination.bmodeOtherAnomalyfDesc}" title="${examination.bmodeOtherAnomalyfDesc}"  style="width: 650px;"  reg='{"maxlength":"100"}'>
                </td>
			</tr>
			<c:if test="${isWoman}">
				<tr>
	...
	<fieldset class="layui-elem-field">
		...
		<colgroup>
			...
			<tr>
			    ...
			    <td>
			        <input type="radio" onclick="util.clickHideTable(this,'ttb9')" name="cvascularFlag" ${examination.cvascularFlag eq "0" or examination.cvascularFlag eq null ?"checked":""} value="0"> 未发现
			        <input type="radio" id="cvascularFlag" onclick="util.clickShowTable(this,'ttb9','cvascularOtherDesc')" name="cvascularFlag" ${examination.cvascularFlag eq"1" ?"checked":""} value="1"> 已发现
			...
			<tr>
			    <th>肾脏疾病</th>
			    <td>
			        <input type="radio" onclick="util.clickHideTable(this,'ttb10')" name="kidneyDiseaseFlag" ${examination.kidneyDiseaseFlag eq "0"  or examination.kidneyDiseaseFlag eq null ?"checked":""} value="0"> 未发现
			        <input type="radio" id="kidneyDiseaseFlag" onclick="util.clickShowTable(this,'ttb10','kidneyOtherDesc')" name="kidneyDiseaseFlag" ${examination.kidneyDiseaseFlag eq"1" ?"checked":""} value="1"> 已发现
			...
			<tr>
			    <th>心脏疾病</th>
			    <td>
			        <input type="radio" onclick="util.clickHideTable(this,'ttb11')" name="heartDiseaseFlag" ${examination.heartDiseaseFlag eq "0" or examination.heartDiseaseFlag eq null ?"checked":""} value="0"> 未发现
			        <input type="radio" id="heartDiseaseFlag" onclick="util.clickShowTable(this,'ttb11','heartOtherDesc')" name="heartDiseaseFlag" ${examination.heartDiseaseFlag eq"1" ?"checked":""} value="1"> 已发现
			...
			<tr>	
			    <th>血管疾病</th>
			    <td>
			        <input type="radio" onclick="util.clickHideTable(this,'ttb12')" name="arteryDiseaseFlag" ${examination.arteryDiseaseFlag eq "0" or examination.arteryDiseaseFlag eq null ?"checked":""} value="0"> 未发现
			        <input type="radio" id="arteryDiseaseFlag" onclick="util.clickShowTable(this,'ttb12','arteryOtherDesc')" name="arteryDiseaseFlag" ${examination.arteryDiseaseFlag eq"1" ?"checked":""} value="1"> 已发现
			...
			<tr>
			    <th>眼部疾病</th>
			    <td>
			        <input type="radio" onclick="util.clickHideTable(this,'ttb13')" name="eyeDiseasesFlag" ${examination.eyeDiseasesFlag eq "0" or examination.eyeDiseasesFlag eq null ?"checked":""} value="0"> 未发现
			        <input type="radio" id="eyeDiseasesFlag" onclick="util.clickShowTable(this,'ttb13','eyeOtherDesc')" name="eyeDiseasesFlag" ${examination.eyeDiseasesFlag eq"1" ?"checked":""} value="1"> 已发现
			...
			<tr style="display: none">
			    <th>高血压</th>
			    <td>
			        <input type="radio" id="hypertensionFlagNotFind" onclick="util.clickHideText(this,'hypertensionDesc');util.clickHideText(this,'hypertensionLevelTr')" name="hypertensionFlag" ${examination.hypertensionFlag eq "0" or examination.hypertensionFlag eq null ? "checked" : ""} value="0"> 未发现
			        <input type="radio" id="hypertensionFlag" onclick="util.clickShowText(this,'hypertensionDesc');util.clickShowText(this,'hypertensionLevelTr')" name="hypertensionFlag" ${examination.hypertensionFlag eq"1" ?"checked":""} value="1"> 有
			...
			<tr style="display: none">
			    <th>糖尿病</th>
			    <td>
			        <input type="radio" id="diabetesMellitusFlagNotFind" onclick="util.clickHideText(this,'diabetesMellituDesc');util.clickHideText(this,'bloodGluAssessmentTr')" name="diabetesMellitusFlag" ${examination.diabetesMellitusFlag eq "0" or examination.diabetesMellitusFlag eq null ? "checked" : ""} value="0"> 未发现
			        <input type="radio" id="diabetesMellitusFlag" onclick="util.clickShowText(this,'diabetesMellituDesc');util.clickShowText(this,'bloodGluAssessmentTr')" name="diabetesMellitusFlag" ${examination.diabetesMellitusFlag eq"1" ?"checked":""} value="1"> 有
	...
	<fieldset class="layui-elem-field">
		...
		<table class="posttable">
			<tr>
				...
				<td>
			        <label><input type="radio" name="PersonalPhyExamDTO.hospitalizedHistoryFlg" ${hospitalizedHistoryFlg eq "0" or hospitalizedHistoryFlg eq null? "checked":""} onclick="util.clickHideTable(this,'ttb20')" value="0"> 无</label>
			        <label><input type="radio" id="hospitalizedHistoryFlg" name="PersonalPhyExamDTO.hospitalizedHistoryFlg" ${hospitalizedHistoryFlg eq "1" ? "checked":""} onclick="util.clickShowTable(this,'ttb20')" value="1"> 有</label>
			                ...
			                <table id="ttb20" class="tt_hidden">
			                        ...
			                        <td style="text-align:center;">
			                           <%-- <tag:dateInput reg="{'dependOn':'hospitalizedHistoryFlg','required':true}" onlypast="true" style="width: 70px;" id="hospitalizedHistoryList0inDate" name="PersonalPhyExamDTO.hospitalizedHistoryList[0].inDate" ></tag:dateInput>
			                            ...
			                            <input type="text" reg="{'dependOn':'hospitalizedHistoryFlg','required':true}" class="layui-input x-admin-content-sm-date"  name="PersonalPhyExamDTO.hospitalizedHistoryList[0].inDate" id="hospitalizedHistoryList0inDate" 
			                             ...
			                             / 
			                              <input type="text" reg='{"dependOn":"hospitalizedHistoryFlg","required":true,"compare":["hospitalizedHistoryList0inDate","ge","结束日期不能小于开始日期"]}' class="layui-input x-admin-content-sm-date"  name="PersonalPhyExamDTO.hospitalizedHistoryList[0].outhosDate" id="hospitalizedHistoryList0outDate" 
			                            style="padding-left: 0px;width: 70px;" />
			                        </td>
			                        <td style="text-align:center;">
			                    ...
			                    <tr>
			                       ...
			                       <input type="text"  class="layui-input x-admin-content-sm-date"  name="PersonalPhyExamDTO.hospitalizedHistoryList[1].inDate" id="hospitalizedHistoryList1inDate" 
			                             style="padding-left: 0px;width: 70px;" />
			                             / 
			                              <input type="text" reg='{"compare":["hospitalizedHistoryList1inDate","ge","结束日期不能小于开始日期"]}' class="layui-input x-admin-content-sm-date"  name="PersonalPhyExamDTO.hospitalizedHistoryList[1].outhosDate" id="hospitalizedHistoryList1outDate" 
			                            style="padding-left: 0px;width: 70px;" />
			                        </td>
			                        <td style="text-align:center;">
			                    ...
			                    <tr>
			                        <td style="text-align:center;">
			                         ...
			                         <input type="text" class="layui-input x-admin-content-sm-date"  name="PersonalPhyExamDTO.hospitalizedHistoryList[2].inDate" id="hospitalizedHistoryList2inDate"
			                             style="padding-left: 0px;width: 70px;" />
			                             / 
			                              <input type="text" reg='{"compare":["hospitalizedHistoryList2inDate","ge","结束日期不能小于开始日期"]}' class="layui-input x-admin-content-sm-date"  name="PersonalPhyExamDTO.hospitalizedHistoryList[2].outhosDate" id="hospitalizedHistoryList2outDate" 
			                            style="padding-left: 0px;width: 70px;" />
			                        </td>
			                        <td style="text-align:center;">
			                 ...
			                 <c:forEach items="${hospitalizedHistoryList}" var="hospitalizedList" varStatus="status">
			                     <tr>
			                     	<c:if test="${status.index eq 0}">
				                         ...
				                         <input type="text" reg="{'dependOn':'hospitalizedHistoryFlg','required':true}" class="layui-input x-admin-content-sm-date"  name="PersonalPhyExamDTO.hospitalizedHistoryList[${status.index}].inDate" id="hospitalizedHistoryList${status.index}inDate" value="<fmt:formatDate value='${hospitalizedList.inDate}' pattern='yyyy/MM/dd'/>"
			                             style="padding-left:0px;width: 70px;" />
			                             / 
			                              <input type="text" reg='{"dependOn":"hospitalizedHistoryFlg","required":true,"compare":["hospitalizedHistoryList${status.index}inDate","ge","结束日期不能小于开始日期"]}' class="layui-input x-admin-content-sm-date"  name="PersonalPhyExamDTO.hospitalizedHistoryList[${status.index}].outhosDate" id="hospitalizedHistoryList${status.index}outDate" value="<fmt:formatDate value='${hospitalizedList.outhosDate}' pattern='yyyy/MM/dd'/>"
			                            style="padding-left:0px;width: 70px;" />
				                         </td>
				                         <td style="text-align:center;">
			                        ...
			                        <c:if test="${status.index ne 0}">
				                          ...
				                          --%>
				                          <input type="text"  class="layui-input x-admin-content-sm-date"  name="PersonalPhyExamDTO.hospitalizedHistoryList[${status.index}].inDate" id="hospitalizedHistoryList${status.index}inDate" value="<fmt:formatDate value='${hospitalizedList.inDate}' pattern='yyyy/MM/dd'/>"
			                             style="padding-left:0px;width: 70px;" />
			                             / 
			                              <input type="text" reg='{"compare":["hospitalizedHistoryList${status.index}inDate","ge","结束日期不能小于开始日期"]}' class="layui-input x-admin-content-sm-date"  name="PersonalPhyExamDTO.hospitalizedHistoryList[${status.index}].outhosDate" id="hospitalizedHistoryList${status.index}outDate" value="<fmt:formatDate value='${hospitalizedList.outhosDate}' pattern='yyyy/MM/dd'/>"
			                            style="padding-left:0px;width: 70px;" />
				                         </td>
				                         <td style="text-align:center;">
			 ...
			 <tr>
				<th>家庭病床史</th>
				<td>
			        <label><input type="radio" name="PersonalPhyExamDTO.familyBedHistoryFlg" ${familyBedHistoryFlg eq '0' or familyBedHistoryFlg eq null ? 'checked':''} onclick="util.clickHideTable(this,'ttb21')" value="0"> 无</label>
			        <label><input type="radio" id="familyBedHistoryFlg" name="PersonalPhyExamDTO.familyBedHistoryFlg" ${familyBedHistoryFlg eq '1' ? 'checked':''} onclick="util.clickShowTable(this,'ttb21')" value="1"> 有</label>
			        <c:if test="${empty examination.id || empty familyBedHistoryList }">
			            <table id="ttb21" class="tt_hidden">
			                ...
			                <tr>
			                    ...
			                    --%> 
			                    <input type="text"  class="layui-input x-admin-content-sm-date"  name="PersonalPhyExamDTO.familyBedHistoryList[1].builtBedDate" id="familyBedHistoryList1builtBedDate"
			                             style="padding-left: 0px;width: 70px;" />
			                             / 
			                              <input type="text" reg='{"compare":["familyBedHistoryList1builtBedDate","ge","结束日期不能小于开始日期"]}' class="layui-input x-admin-content-sm-date"  name="PersonalPhyExamDTO.familyBedHistoryList[1].removeBedDate" id="familyBedHistoryList1removeBedDate" 
			                            style="padding-left: 0px;width: 70px;" />
			                    </td>
			                    <td style="text-align:center;">
			                ...
			                <tr>
			                    <td style="text-align:center;">
			                     ...
			                     --%>
			                      <input type="text"  class="layui-input x-admin-content-sm-date"  name="PersonalPhyExamDTO.familyBedHistoryList[2].builtBedDate" id="familyBedHistoryList2builtBedDate"
			                             style="padding-left: 0px;width: 70px;" />
			                             / 
			                              <input type="text" reg='{"compare":["familyBedHistoryList1bui2tBedDate","ge","结束日期不能小于开始日期"]}' class="layui-input x-admin-content-sm-date"  name="PersonalPhyExamDTO.familyBedHistoryList[2].removeBedDate" id="familyBedHistoryList2removeBedDate" 
			                            style="padding-left: 0px;width: 70px;" />
			                    </td>
			                    <td style="text-align:center;">
			                    ...
			                    <tr>
			                    	<c:if test="${status.index eq 0}">
				                        <td style="text-align:center;">
				                         ...
				                         --%>
				                        <input type="text" reg="{'dependOn':'familyBedHistoryFlg','required':true}" class="layui-input x-admin-content-sm-date"  name="PersonalPhyExamDTO.familyBedHistoryList[${status.index}].builtBedDate" id="familyBedHistoryList${status.index}builtBedDate" value="<fmt:formatDate value='${familyBed.builtBedDate}' pattern='yyyy/MM/dd'/>"
			                             style="padding-left: 0px;width: 70px;" />
			                             / 
			                              <input type="text" reg='{"dependOn":"familyBedHistoryFlg","required":true,"compare":["familyBedHistoryList${status.index}builtBedDate","ge","结束日期不能小于开始日期"]}' class="layui-input x-admin-content-sm-date"  name="PersonalPhyExamDTO.familyBedHistoryList[${status.index}].removeBedDate" id="familyBedHistoryList${status.index}removeBedDate" 
			                        ...
			                        <c:if test="${status.index ne 0}">
				                       <td style="text-align:center;">
				                         ...
				                         --%>
				                         <input type="text" class="layui-input x-admin-content-sm-date"  name="PersonalPhyExamDTO.familyBedHistoryList[${status.index}].builtBedDate" id="familyBedHistoryList${status.index}builtBedDate" value="<fmt:formatDate value='${familyBed.builtBedDate}' pattern='yyyy/MM/dd'/>"
			                             style="padding-left: 0px;width: 70px;" />
			                             / 
			                              <input type="text" reg='{"compare":["familyBedHistoryList${status.index}builtBedDate","ge","结束日期不能小于开始日期"]}' class="layui-input x-admin-content-sm-date"  name="PersonalPhyExamDTO.familyBedHistoryList[${status.index}].removeBedDate" id="familyBedHistoryList${status.index}removeBedDate" 
			                              value="<fmt:formatDate value='${familyBed.removeBedDate}' pattern='yyyy/MM/dd'/>"
			                            style="padding-left: 0px;width: 70px;" />
				                        </td>
				                        <td style="text-align:center;">
		        	...
		        	<table id="ttb22" class="tt_hidden">
		        		...
		        		<tr>
		        			...
		        			<input type="text"  class="layui-input x-admin-content-sm-date"  name="PersonalPhyExamDTO.drugHistoryList[4].startDate" id="drugHistoryList4startDate" value="<fmt:formatDate value='${drugHistoryList[4].startDate}' pattern='yyyy/MM/dd'/>"
			                             style="padding-left: 0px;width: 70px;" />
			                             / 
			                              <input type="text" reg='{"compare":["drugHistoryList4startDate","ge","结束日期不能小于开始日期"]}' class="layui-input x-admin-content-sm-date"  name="PersonalPhyExamDTO.drugHistoryList[4].stopDate"  id="drugHistoryList4startDate"
...
<div class="postdiv">
	<fieldset class="layui-elem-field">
		...
		<div class="toolbarsublist">
			...
			<input type="hidden" id="personId" value="${personInfo.id}" name="personInfo.id">
		...
		<table class="posttable" id="CMedicine">
			<tr>
			    ...
			    <td>
                        <ehr:dic-radio  name="tcmPeacefulQuality" dicmeta="FS10186" code="1,2,0" value="${examination.tcmPeacefulQuality}" disabled="disabled"></ehr:dic-radio>
			    </td>
			</tr>
			<tr>
			    <th>气虚质</th>
			    <td>
                        <ehr:dic-radio  name="tcmQiQuality" dicmeta="FS10186" code="1,3,0" value="${examination.tcmQiQuality}" disabled="disabled"></ehr:dic-radio>
			    </td>
			</tr>
			<tr>
			    <th>阳虚质</th>
			    <td>
                    <ehr:dic-radio  name="tcmYangQuality" dicmeta="FS10186" code="1,3,0" value="${examination.tcmYangQuality}" disabled="disabled"></ehr:dic-radio>
			    </td>
			</tr>
			<tr>
			    <th>阴虚质</th>
			    <td>
                        <ehr:dic-radio  name="tcmYinDeficiency" dicmeta="FS10186" code="1,3,0" value="${examination.tcmYinDeficiency}" disabled="disabled"></ehr:dic-radio>
			    </td>
			</tr>
			<tr>
			    <th>痰湿质</th>
			    <td>
                        <ehr:dic-radio  name="tcmPhlegmWetness" dicmeta="FS10186" code="1,3,0" value="${examination.tcmPhlegmWetness}" disabled="disabled"></ehr:dic-radio>
			    </td>
			</tr>
			<tr>
			    <th>湿热质</th>
			    <td>
                        <ehr:dic-radio  name="tcmHeatMedium" dicmeta="FS10186" code="1,3,0" value="${examination.tcmHeatMedium}" disabled="disabled"></ehr:dic-radio>
			     </td>
			</tr>
			<tr>
			    <th>血瘀质</th>
			    <td>
                        <ehr:dic-radio  name="tcmBloodQuality" dicmeta="FS10186" code="1,3,0" value="${examination.tcmBloodQuality}" disabled="disabled"></ehr:dic-radio>
                </td>
			</tr>
			<tr>
			    <th>气郁质</th>
			    <td>
                        <ehr:dic-radio  name="tcmQiStagnation" dicmeta="FS10186" code="1,3,0" value="${examination.tcmQiStagnation}" disabled="disabled"></ehr:dic-radio>
                </td>
			</tr>
			<tr>
			    <th>特秉质</th>
			    <td>
                        <ehr:dic-radio  name="tcmSpecialQuality" dicmeta="FS10186" code="1,3,0" value="${examination.tcmSpecialQuality}" disabled="disabled"></ehr:dic-radio>
                </td>
			</tr>
		</table>
	</fieldset>
	...
	<fieldset class="layui-elem-field">
        ...
        <table class="posttable">
            ...
            <tr>
                ...
                <td>
                    <ehr:dic-radio name="healthSelfAssessment" value="${examination.healthSelfAssessment}"
                                   dicmeta="CV0401013" reg="{'required':true}"/>
                </td>
            </tr>
            <tr>
                <th><label class="required">生活自理能力自我评估</label></th>
                <td>
                    <ehr:dic-radio name="lifeAbilitySelfAssessment" value="${examination.lifeAbilitySelfAssessment }"
                                   dicmeta="CV0401014" reg="{'required':true}"/>
                </td>
            </tr>
            <tr>
                <th><label class="required">进餐：使用餐具将饭菜送入口、咀嚼、吞咽等活动</label></th>
                <td>
                    <label><input type="radio" name="eatingAssessment"
                                  onclick="cdmPersonPhyExamAdd.calculateAssessment()" value="0"
                    ${examination.eatingAssessment eq 0 ? "checked" : "" } reg="{'required':true}"/> 独立完成（0）</label>
                    <label><input type="radio" name="eatingAssessment"
                                  onclick="cdmPersonPhyExamAdd.calculateAssessment()" value="3"
                    ${examination.eatingAssessment eq 3 ? "checked" : ""} reg="{'required':true}"/> 需要协助（3）</label>
                    <label><input type="radio" name="eatingAssessment"
                                  onclick="cdmPersonPhyExamAdd.calculateAssessment()" value="3"
                    ${examination.eatingAssessment eq 5 ? "checked" : ""} reg="{'required':true}"/> 完全需要帮助（5）</label>
                </td>
            </tr>
            <tr>
                <th><label class="required">梳洗：梳头、洗脸、刷牙、剃须、洗澡等活动</label></th>
                <td>

                    <label><input type="radio" name="cleaningAssessment"
                                  onclick="cdmPersonPhyExamAdd.calculateAssessment()" value="0"
                    ${examination.cleaningAssessment eq 0 ? "checked" : "" } reg="{'required':true}"/> 独立完成（0）</label>
                    <label><input type="radio" name="cleaningAssessment"
                                  onclick="cdmPersonPhyExamAdd.calculateAssessment()" value="1"
                    ${examination.cleaningAssessment eq 1 ? "checked" : ""} reg="{'required':true}"/> 洗澡需协助（1）</label>
                    <label><input type="radio" name="cleaningAssessment"
                                  onclick="cdmPersonPhyExamAdd.calculateAssessment()" value="3"
                    ${examination.cleaningAssessment eq 3 ? "checked" : ""} reg="{'required':true}"/> 协助下能完成（3）</label>
                    <label><input type="radio" name="cleaningAssessment"
                                  onclick="cdmPersonPhyExamAdd.calculateAssessment()" value="7"
                    ${examination.cleaningAssessment eq 7 ? "checked" : ""} reg="{'required':true}"/> 完全需要帮助（7）</label>
            ...
            <tr>
                ...
                <td>
                    <label><input type="radio" name="clothingAssessment"
                                  onclick="cdmPersonPhyExamAdd.calculateAssessment()" value="0"
                    ${examination.clothingAssessment eq 0 ? "checked" : ""} reg="{'required':true}"/> 独立完成（0）</label>
                    <label><input type="radio" name="clothingAssessment"
                                  onclick="cdmPersonPhyExamAdd.calculateAssessment()" value="3"
                    ${examination.clothingAssessment eq 3 ? "checked" : ""} reg="{'required':true}"/> 需要协助（3）</label>
                    <label><input type="radio" name="clothingAssessment"
                                  onclick="cdmPersonPhyExamAdd.calculateAssessment()" value="5"
                    ${examination.clothingAssessment eq 5 ? "checked" : ""} reg="{'required':true}"/> 完全需要帮助（5）</label>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <label><input type="radio" name="defecationAssessment"
                                  onclick="cdmPersonPhyExamAdd.calculateAssessment()" value="0"
                    ${examination.defecationAssessment eq 0 ? "checked" : ""} reg="{'required':true}"/> 不需协助（0）</label>
                    <label><input type="radio" name="defecationAssessment"
                                  onclick="cdmPersonPhyExamAdd.calculateAssessment()" value="1"
                    ${examination.defecationAssessment eq 1 ? "checked" : ""} reg="{'required':true}"/> 偶尔失禁（1）</label>
                    <label><input type="radio" name="defecationAssessment"
                                  onclick="cdmPersonPhyExamAdd.calculateAssessment()" value="5"
                    ${examination.defecationAssessment eq 5 ? "checked" : ""} reg="{'required':true}"/> 经常失禁（5）</label>
                    <label><input type="radio" name="defecationAssessment"
                                  onclick="cdmPersonPhyExamAdd.calculateAssessment()" value="10"
                    ${examination.defecationAssessment eq 10 ? "checked" : ""} reg="{'required':true}"/>
                        完全失禁（10）</label>
                </td>
            </tr>
            <tr>
                <th><label class="required">活动：站立、室内行走、上下楼梯、户外活动</label></th>
                <td>
                    <label><input type="radio" name="exerciseAssessment"
                                  onclick="cdmPersonPhyExamAdd.calculateAssessment()" value="0"
                    ${examination.exerciseAssessment eq 0 ? "checked" : ""} reg="{'required':true}"/> 独立完成（0）</label>
                    <label><input type="radio" name="exerciseAssessment"
                                  onclick="cdmPersonPhyExamAdd.calculateAssessment()" value="1"
                    ${examination.exerciseAssessment eq 1 ? "checked" : ""} reg="{'required':true}"/> 借助较小外力（1）</label>
                    <label><input type="radio" name="exerciseAssessment"
                                  onclick="cdmPersonPhyExamAdd.calculateAssessment()" value="5"
                    ${examination.exerciseAssessment eq 5 ? "checked" : ""} reg="{'required':true}"/> 借助较大外力（5）</label>
                    <label><input type="radio" name="exerciseAssessment"
                                  onclick="cdmPersonPhyExamAdd.calculateAssessment()" value="10"
                    ${examination.exerciseAssessment eq 10 ? "checked" : ""} reg="{'required':true}"/> 卧床不起（10）</label>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <label><input type="radio" name="cognitionScreenResult"
                        ...
                        粗筛阴性</label>
                    <label><input type="radio" id="cognitionScreenResult" name="cognitionScreenResult"
                    ...
                    <span id="cognitionScreenScore"  ${examination.cognitionScreenResult ne'0' ?'class="hidediv"' :''}>
						   ，简易智力状态检查，总分<tag:numberInput name="cognitionScreenScore" style="width:40px;"
                                                        value="${examination.cognitionScreenScore}" cssClass="width30"
            ...
            <tr>
                ...
                <td>
                    <label><input type="radio" name="emotionScreenResult"
                                  ...
                                  value="1" reg="{'required':true}"/>
                        粗筛阴性</label>
                    <label><input type="radio" id="emotionScreenResult" name="emotionScreenResult"
                    ...
                    <span id="depressionScore" ${examination.emotionScreenResult ne'0' ?'class="hidediv"' :''}>
						，老年人抑郁评分检查，总分<tag:numberInput name="depressionScore" id="depressionScore"
		    			...
		    			<a href="javascript:void(0)" id="addOldDepressed"><b class="xinz">老年抑郁</b></a>
					</span>							   
					</span>
                </td>
            </tr>
...
<div class="postdiv">
	<fieldset class="layui-elem-field">
		...
		<table class="posttable">
			...
			<tr>
				<th><label class="required">健康评价</label></th>
				<td>
					<label><input type="radio" id="healthEvaluateAnomalyFlgN"
						   onclick="util.clickHideTable(this,'ttb30')"  ${examination.healthEvaluateAnomalyFlag eq '0'?'checked ="checked"':''}
						   name="healthEvaluateAnomalyFlag" value="0" reg="{'required':true}"> 体检无异常（结合临床表现综合评估）</label>
						<label><input type="radio" id="healthEvaluateAnomalyFlg" ${examination.healthEvaluateAnomalyFlag eq '1'?'checked ="checked"':''}
						   onclick="util.clickShowTable(this,'ttb30')"
						   name="healthEvaluateAnomalyFlag" value="1" reg="{'required':true}"> 有异常</label>
					<br/>
					<span class="hidediv" id="ttb30">
						  异常 <input id="healthAbnormal1" name="healthEvaluateAnomaliyList" value="${healthEvaluateAnomalyList[0].healthEvaluateAnomalyDesc}" reg='{"dependOn":"healthEvaluateAnomalyFlg","required":true,"maxlength":"100"}' style="width: 400px; border: none; border-bottom: 1px solid #D7D7D7;"> <br>
						  异常 <input id="healthAbnormal2" name="healthEvaluateAnomaliyList" value="${healthEvaluateAnomalyList[1].healthEvaluateAnomalyDesc}" reg='{"maxlength":"100"}' style="width: 80%; border: none; border-bottom: 1px solid #D7D7D7;"> <br>
						  异常 <input id="healthAbnormal3" name="healthEvaluateAnomaliyList" value="${healthEvaluateAnomalyList[2].healthEvaluateAnomalyDesc}" reg='{"maxlength":"100"}' style="width: 80%; border: none; border-bottom: 1px solid #D7D7D7;"> <br>
						  异常 <input id="healthAbnormal4" name="healthEvaluateAnomaliyList" value="${healthEvaluateAnomalyList[3].healthEvaluateAnomalyDesc}" reg='{"maxlength":"100"}' style="width: 80%; border: none; border-bottom: 1px solid #D7D7D7;">
                        <%--异常 <input id="healthAbnormal3" name="healthEvaluateAnomaliyList" value="" reg='{"maxlength":"33"}' style="width: 400px; border: none; border-bottom: 1px solid #D7D7D7;">--%>
					</span>
				</td>
			</tr>
			<tr>
                <th title="《考核项目》纳入慢性病患者健康管理是指高血压、糖尿病、严重精神障碍患者等重点人群定期随访和健康体检。减体重的目标是指根据居民或患者的具体情况,指定下次体检之前需要减重的目标值。" style="width: 16%"><label class="required">健康指导</label></th>
				<td id="ttbhealth">
					<%--<label>
						<input type="checkbox"
			...
			<tr>
				<th><label class="required">危险因素控制</label></th>
				<td id="ttbRisk">
					<label>
						<input type="checkbox" reg='{"extension":["riskFactorVali","请至少选择一项"]}'
							   name="riskQuitSmoking" ${examination.riskQuitSmoking eq '1' ? 'checked' : ''} value="1"/>
						<span>戒烟</span>
					</label>
					<br>
					<label>
						<input type="checkbox" reg='{"extension":["riskFactorVali","请至少选择一项"]}'
							   name="riskHealthDrink" ${examination.riskHealthDrink eq '1' ? 'checked' : ''} value="1"/>
						<span>健康饮酒</span>
					</label>
					<br>
					<label>
						<input type="checkbox" name="riskDiet" ${examination.riskDiet eq '1' ? 'checked' : ''} reg='{"extension":["riskFactorVali","请至少选择一项"]}'
							   value="1"/>
						<span>饮食</span>
					</label>
					<br>
					<label>
						<input type="checkbox" name="riskExercise" ${examination.riskExercise eq '1' ? 'checked' : ''} reg='{"extension":["riskFactorVali","请至少选择一项"]}'
							   value="1"/>
						<span>锻炼</span>
					</label>
					<br>
					<label>
						<input type="checkbox" id="riskWeightReduction" onclick="util.clickShowText(this,'riskWeightTarget')" reg='{"extension":["riskFactorVali","请至少选择一项"]}'
							   name="riskWeightReduction" ${examination.riskWeightReduction eq '1' ? 'checked' : ''}
							   value="1"/>
						<span>减体重</span>
					</label>
					<span id="riskWeightTarget" class="hidediv">目标:&nbsp;
						 <!--  <input type="text" name="PersonalPhyExamDTO.examination.riskWeightTarget" value="${examination.riskWeightTarget}">  -->
						 <tag:numberInput point="point" value="${examination.riskWeightTarget}" name="riskWeightTarget"
										  style="width: 40px;"
										  reg="{'dependOn':'riskWeightReduction','scale':2,'required':'true','min':0,'max':9999}"/>Kg
					</span>
					<br>
					<label>
						<input type="checkbox" id="guideVaccination" onclick="util.clickShowText(this,'guideVaccinationDesc')" reg='{"extension":["riskFactorVali","请至少选择一项"]}'
					...
					<br>
					<label>
						<input type="checkbox" id="riskOther" onclick="util.clickShowText(this,'riskOtherDesc')"  reg='{"extension":["riskFactorVali","请至少选择一项"]}'
							   name="riskOther" ${examination.riskOther eq '1' ? 'checked' : ''} value="1"/>
						<span>其他</span></label>
 ...
 var fbsize = 3;
    layui.use('laydate', function(){
        ...
        for (var i = 0; i < hhsize; i++) {
			...
			var hhoutdateId = 'hospitalizedHistoryList' + i + 'outDate';
	        laydate.render({
	            elem: '#' + hhIndateId 
	         	   ,format: 'yyyy/MM/dd'
	         	   ,max:0

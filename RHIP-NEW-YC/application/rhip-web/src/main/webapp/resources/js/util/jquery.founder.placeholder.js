(function($)
{
    ...
    $.Placeholder = {
        ...
        init: function (setting) {
            $.extend(this.defSettings, setting);
            var $target = $(this.defSettings.query);
            if (isSupport) {
                var callback=this.defSettings.callback;
                $target.on('blur', function (ev) {
                    if ($target.val() && $.isFunction(callback)) {
                        callback($target);
                    }
                });
            } else {
                $target.placeholder(this.defSettings.callback);
            }
        }
    }


    //判断ie版本,ie11无法用此判断,直接为999

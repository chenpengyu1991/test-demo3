...
{ 
	...
	if(typeof setting.Cert == "object")
	{
		...
		ZK.extend(Cert, Device, {
			...
			interfaceResult : 
			{
				...
				2:{mean:"数据传输超时",message:"请将二代身份证放到读卡区域...",type:"error"},
				10:{mean:"没有找到卡",message:"未读取到身份证，请重新刷卡！",type:"warning"},
				11:{mean:"读卡操作失败",message:"未读取到身份证，请重新刷卡！",type:"error"},
	...
	Device.prototype.xDomainAccess = function(url)
	{
		...
		{ 
			...
			xDomainRequest.open('GET', issOnlineUrlReadCard + url);
	...
	//增加读卡后自动查询功能
    function notNulSearch(textIdcard,searchBtn){
        // if($("#text_idcard").val()!="")
        //     $('#per_search_btn').click();
        if($("#"+text_idcard).val()!="")
            $('#'+search_btn).click();
    }
    function readAndSearch(textIdcard,searchBtn){
        setTimeout("notNulSearch("+textIdcard+","+searchBtn+");",1000);
    }

	Device.prototype.startFun = function(text_idcard,search_btn)
	{
		//创建设备
...
function DoGetSNR()
{
	$.ajax( {
		type : "GET",
		url : "http://127.0.0.1:24010/ZKIDROnline/MFCard?OP-DEV=1&CMD-URL=1&iPort=7",
		dataType : "json",
		async: true,
		success : function(data) 
		{

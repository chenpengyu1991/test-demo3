...
(function ($) {
    "use strict";
    var loadingSource = "<img src='" + contextPath + "/images/AjaxLoader24.gif' />";

    var exportExcelUrl = "/export/excel";

    var doExportExcelUrl = contextPath + '/export/excel/';

    var cleanRegex = new RegExp("/(\n\s*)|\n|\r|(\r\n)/", "g");

    $.fn.exportExcel = function (title) {
        var table = parseTable(this);
        table = clean(table);
        var option = {
            url: exportExcelUrl,
            param: {
                title: getTitle(this, title),
                content: table
            },
            callback: doExportExecl
        };
        $.getJsonByUrl(option);
    };

    function clean(source) {
        var result = source.replace(cleanRegex, "");
        return result;
    }
    ...
    function parseTable($table) {
        var table = '<?xml version="1.0" encoding="UTF-8"?>' + "<table>";
        table += getPart($table, "thead");
        table += getPart($table, "tbody");
        table += getPart($table, "tfoot");
        table += "</table>";
        return table;
    }

    function getPart($table, part) {
        var table = "<" + part + ">";
        var $header = $table.find(part);
        if ($header.length > 0) {
            table += $header.html();
        }
        table += "</" + part + ">";
    ...
    function doExportExecl(data) {
        if (data) {
            var url = doExportExcelUrl + data;
            download(url);
        }
    }
    ...
    var blockUiSetting=    {
        ...
        message: loadingSource + "<div >excel生成中... 数据较多时会消耗较长时间,请耐心等待!</div>"
    };

    function download(url) {
        // var $iframe = $("<iframe >");
        // $iframe.hide().prop("src", url).appendTo("body");
        //模态提示
        ...
        fileDownload(url, {
            ...
            failCallback: function (responseHtml, url) {
                ...
                layui.use('layer', function() {
           			...
           			layer.alert("导出失败,请联系管理员...", {icon:0,title:'提示'});
              	});
            },
            resposeCallback: function (responseHtml, url) {
                $.unblockUI();
                layui.use('layer', function() {
           			var layer = layui.layer;
           			layer.alert("导出操作人数太多,请稍候再试...", {icon:0,title:'提示'});
              	});
            }
        });
    }

    $.fn.exportListExcel = function (option) {
        option = option || {};
        var formParam = this.serializeObject();
        var url = option.url;
        var param = option.param || {};
        var allParam = $.extend(formParam, param);
        var mergedUrl = getMergedUrl(url, allParam);
        download(mergedUrl);
    };

    function getMergedUrl(url, allParam) {
        return contextPath + url + "?" + $.param(allParam, true);
    }
     ...
     *
     * 导出下载插件
     * 修改自http://johnculviner.com/jquery-file-download-plugin-for-ajax-like-feature-rich-file-downloads/
     * 请勿直接替换
     */
    ...
    function fileDownload(fileUrl, options) {
        ...
        var internalCallbacks = {
            ...
            // 返回提示
            //
            onRespose: function (responseHtml, url) {
                ...
                settings.resposeCallback(responseHtml, url);
        ...
        function checkFileDownloadComplete() {
            ...
            if (document.cookie.indexOf(settings.cookieName + "=false") != -1) {
                ...
                // execute specified callback
                internalCallbacks.onRespose("", fileUrl);
                // remove the cookie and iframe
                document.cookie = settings.cookieName + "=; expires=" + new Date(1000).toUTCString() + "; path=" + settings.cookiePath;
                cleanUp(false, true);
                return;
        ...
        function cleanUp(isFailure, removeIframe) {
            ...
            setTimeout(function () {
                ...
                if (downloadWindow) {
                    ...
                    if (isIos) {
                        downloadWindow.focus(); // ios safari bug doesn't allow
                        // a window to be closed unless
                        // it is focused
                        if (isFailure) {
                            downloadWindow.close();
                        }

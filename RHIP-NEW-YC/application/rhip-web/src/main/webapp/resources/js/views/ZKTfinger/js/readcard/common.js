var issOnlineUrlReadCard = "http://127.0.0.1:24010/ZKIDROnline";
	...
	var setting = {
			...
			Methods : {
				showMessage : function(type,message){
					var id_str ="add_cert_message";
					if($("#"+id_str).length<=0)
                        id_str ="cert_message"
                    $("#"+id_str).show();
					if("success"==type){
                        $("#"+id_str).removeClass("msgError");
                        $("#"+id_str).addClass("msgok");
					}
					if("error"==type){
                        $("#"+id_str).addClass("msgError");
                        $("#"+id_str).removeClass("msgok");
                    }
                    $("#"+id_str).text(message);
                    if("success"==type)
                        $("#"+id_str).delay(3000).fadeOut();
                    $("#"+id_str+"_type").text(msgType[type]);
				},
				downloadDrive : function(){
					// $.jBox.closeTip();
					messageBox({messageType: "confirm", text: "请安装相关硬件驱动！点击确定下载驱动。",
						callback: function(){
                            window.location.href = contextPath+"/js/views/ZKTfinger/middleware/ZKIDROnline.exe";
							// closeMessage();
					}});
				}
			}
...
function setCertificateData(result)
{
    ...
    $("#text_name").val(result.Certificate.Name);
    if($("#text_idcard").length>0)
        $("#text_idcard").val(result.Certificate.IDNumber);
    if($("#search_idcard").length>0)
    	$("#search_idcard").val(result.Certificate.IDNumber);
    if($("#cdm_report_idcard").length>0){
        $("#cdm_report_idcard").val(result.Certificate.IDNumber);
        $("#cdm_report_idcard").blur();
    }
}
  	...
  	switch(this.messageType)
	{
		case "confirm":
			...
			layui.use('layer', function(){
				...
				layer.confirm(paramsJson.text, {icon:1, title:'确认提示'}, function() {
					paramsJson.callback(true);
				});
			});
			/*msgUtil.confirm(paramsJson.text,  function() {
	     	 		paramsJson.callback(true);
			},"确定");*/
			break;
	}

...
function saveVerifyRecord(){
    var submitObj={
        url:"/cdm/standardization/followup/fingerVerify/saveRecord",
        callback:function(data){
            if(data < 0){
                // $("#msgok").hide();
                $("#msgError").text("验证记录保存失败!");
                $("#msgError").show();
                return;
            }/*else{
                $("#msgError").hide();
                $("#msgok").show();
                $("#msgok").text("验证记录保存成功!");
                $("#msgok").delay(3000).fadeOut();
            }*/
        }
    };
    $("#fingerVerifyForm").submitFormGetJson(submitObj);
}
/**
 * 表单提交回调函数
 * @author wenxin
...
//  		drawHandAndArc(context, coordArray, color, coordJson[i].coord.x, coordJson[i].coord.y, i);
 ...
 function drawHandAndArc(context, coordArray, color, x, y, num)
 {
 	...
 	//绘画圆圈
 	showImage(context, "../js/views/ZKTfinger/image/base_fpVerify_clearImage.png", "clearForRegister");
 	//绘画圆弧
 	//else if(num == 12)
 	//{
...
function doVerify()
{
	...
	// var fpTemplate = "";
    var fpTemplate =$("#verifyTemplate").val();
    var regTemplate =$("#fingerTemplate10").val();
	$.ajax( {
		type : "POST",
		...
		success : function(data) 
		{
 			...
 			if(ret == 0)
			{
                ...
                $("#verifyResult").val(data.score>0?"1":"0");
                saveVerifyRecord();
			}
			else
			{
                // callBackFormSubmit("notExist");
                collectTips(globalContext, "指纹接口调用失败", "verification");
                setTimeout("clearImageData()", 1000);
                // beginCapture(globalContext);
				// alert("ret:" + data.ret);
			}
...
function saveFingerInfo(){
    var personId = $("#text_personId").val();
    var idcard = $("#idcard").val();
    if(personId==''||idcard==''){
        $("#msgok").hide();
    	$("#msgError").text("请先保存封面信息,重新采集指纹!");
        $("#msgError").show();
        return;
    }
    $("#finger_personId").val(personId);
    $("#finger_idcard").val(idcard);
    //手指编号
	var fingerId = fingerIdArray.toString();
	if(fingerId!=''&&fingerId.indexOf(",")>=0){
        fingerId = fingerId.replace(",","");
	}
	if(fingerId.length>1){
        $("#msgok").hide();
        $("#msgError").text("只能保存一个指纹记录,请重新采集指纹!");
        $("#msgError").show();
        return;
	}
    $("#fingerId").val(fingerId);
    //指纹模板
    $("#fingerTemplate10").val(templateDataArray.toString());
    var submitObj={
        url:"/personRecord/saveFingerInfo",
        callback:function(data){
            if(data < 0){
                $("#msgok").hide();
                $("#msgError").text("指纹保存失败!");
                $("#msgError").show();
                return;
            }else{
                $("#msgError").hide();
    ...
    $("#fingerInfoForm").submitFormGetJson(submitObj);
}
var mosquitoesMonitorList = (function() {
	$(function() { 
	...
	function viewOrEdit(monitorId,type, index) {
			...
			$("#mainSearchDiv").hide();
			var option = {
					url : "/dmbc/vertor/initMosquitoesMonitorViewModify/"+monitorId,
	...
	function deleteDo(id){
		$.getJsonByUrl({
	    	url : "/dmbc/vertor/delMosquitoesMonitor",
            callback:function(data){
            	layer.alert(data.message, {icon:0,title:'提示'});
            	if (data.result) {
            		mosquitoesMonitorSearch.search($("#currentPage").val());
        		}
    		},
	    	param:{
	    		id:id

var familyStatisticsSearch = (function() {
	var validate = null;
	$(function() {
        $("#familyResultDivExport").on("click", function(event) {
        	event.preventDefault();
            $("#familyResultDiv").exportExcel("家庭档案统计报表");
        });
        $("#month").change(function(){
        	search();
        	});
		validate = $("#familySearchForm").easyValidate();
        /*健康教育统计查询*/
        $("#familySearchForm").onEnter(search, 1);
        $("#familyStatisticsBtnSearch").click(function(e) {
            e.preventDefault();
        	search();
        });
        //search();
	...
	function search() {
		...
		var year = $("#familySearchForm #year").val();
		var month = $("#familySearchForm #month").val();
		var createBeginTime = new Date($("#familySearchForm #createBeginTime").val());
		var createEndTime = new Date($("#familySearchForm #createEndTime").val());
		...
		var searchObj = {
			url : "/familyStatistics/list",
			insertDiv : "familyResultDiv"
		};
		$("#familySearchForm").submitFormLoadHtml(searchObj);
	}

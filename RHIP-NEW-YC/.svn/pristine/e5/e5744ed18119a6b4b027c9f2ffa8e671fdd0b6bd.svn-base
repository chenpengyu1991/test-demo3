    ...
    <table class="layui-table x-admin-sm-table-list-middle">
        ...
        <c:forEach var="woman" items="${womanList}" varStatus="status">
            <tr>
                ...
                <td class="centertd">
                    <a class="basicIndex layui-btn layui-btn-normal layui-btn-xs" style="color: #FFF;font-size: 12px;" href="#" title="查看" onclick="womanSearch.postpartum(${woman.id})"><i class="layui-icon"></i>查看</a>
                    <c:if test="${curCode eq woman.createOrganCode}">
	                    <a class="personModify layui-btn layui-btn-xs" style="color: #FFF;font-size: 12px;" href="#" title="修改" onclick="womanSearch.modifyPostpartum(${woman.id})"><i class="layui-icon"></i>修改</a>
	                    <a class="layui-btn layui-btn-danger layui-btn-xs" style="color: #FFF;font-size: 12px;" href="javascript:void(0);" onclick="womanSearch.deletePostpartum(${woman.id})" title="删除"><i class="layui-icon"></i>删除</a>
                    </c:if>
                </td>
            </tr>
        </c:forEach>

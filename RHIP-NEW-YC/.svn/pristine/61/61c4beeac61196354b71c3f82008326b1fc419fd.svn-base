...
/**
 * Default implementation of the {@link IPropertiesPersister} interface.
 ...
 */
public class DefaultPropertiesPersister implements IPropertiesPersister {

	// Determine whether Properties.load(Reader) is available (on JDK 1.6+)
	private static final boolean loadFromReaderAvailable =
			ReflectUtil.hasMethod(Properties.class, "load", new Class[] {Reader.class});
	...
	private static final boolean storeToWriterAvailable =
			ReflectUtil.hasMethod(Properties.class, "store", new Class[] {Writer.class, String.class});
			...
			if (line.length() > 0) {
				...
				if (firstChar != '#' && firstChar != '!') {
					while (endsWithContinuationMarker(line)) {
						...
						if (nextLine != null) {
							line += StringUtil.trimLeadingWhitespace(nextLine);
						}
					}

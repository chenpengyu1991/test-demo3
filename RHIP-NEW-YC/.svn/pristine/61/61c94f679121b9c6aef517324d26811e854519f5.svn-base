var sewageTreatmentList = (function() {
	$(function() { 
	...
	function viewOrEdit(monitorId,type, index) {
			...
			$("#mainSearchDiv").hide();
			var option = {
					url : "/dmbc/medicalInst/initSewageTreatmentViewModify/"+monitorId,
	...
	function deleteDo(id){
		$.getJsonByUrl({
	    	url : "/dmbc/medicalInst/delSewageTreatment",
            callback:function(data){
            	layer.alert(data.message, {icon:0,title:'提示'});
            	if (data.result) {
            		sewageTreatmentSearch.search($("#currentPage").val());
        		}
    		},
	    	param:{
	    		id:id

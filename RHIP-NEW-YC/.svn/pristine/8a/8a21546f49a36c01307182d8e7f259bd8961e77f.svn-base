...
<input type="text" <%if (null != name) {%> name="<%=name%>" <%}%>
		<%if (null != id) {%> id="<%=id%>" <%}%> <%if (null != cssClass) {%>
	   class="<%=cssClass%>" <%}%> <%if (null != value) {%> value="<%=value%>"
		<%}%> <%if (null != style) {%> style="<%=style%>" <%}%>
		<%if (null != reg) {%> reg="<%=reg.replaceAll("\"", "'")%>" <%}%>
		<%if (null != disabled && disabled) {%> disabled="disabled" <%}%>
	   onfocus="WdatePicker({
		   <%if(null!=startDate){ %>startDate:'<%=startDate%>',<%} %>
		   <%if(null!=minDate){ %>minDate:'<%=minDate%>',<%} %>
			   skin:'whyGreen',oncleared:onDatePickerChanged,onpicked:onDatePickerChanged,dateFmt:'<%=pattern%>'
		   ...
		   <%if(null!=minId){ %>,minDate:'#F{$dp.$D(\'<%=minId%>\')}'<%} %>
			   })"  data-inputtype="date" <%if(null!=onpropertychange){ %>onpropertychange="<%=onpropertychange%>"<%} %> />

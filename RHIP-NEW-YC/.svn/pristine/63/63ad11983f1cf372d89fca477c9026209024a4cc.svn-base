var breakageSearch = (function() {
	var validate=null;
    $(function() {
    	validate = $("#breakageSearchForm").easyValidate();
        $("#breakageBtnSearch").click(function(e) {
            e.preventDefault();
            search(1);
        });
        $("#breakageSearch").onEnter(search, 1);
        search(1); 
        init();
    });
	function init(){
    ...
    function search(pageIndex) {
        ...
        var searchObj = {
            url : "/da/consumable/breakage/list",
            insertDiv : "breakageResultDiv",
            param : {
                pageIndex : pageIndex
            },
        ...
        $("#breakageSearchForm").submitFormLoadHtml(searchObj);
    };	
	return {
		search:search
	};

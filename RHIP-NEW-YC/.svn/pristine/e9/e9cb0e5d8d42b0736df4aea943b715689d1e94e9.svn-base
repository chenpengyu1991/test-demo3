...
public class CachedIntrospectionResults {
	...
	private static final Logger logger = Logger.getLogger(CachedIntrospectionResults.class);
	...
	static CachedIntrospectionResults forClass(Class<?> beanClass) throws BaseException {
		...
		if (results == null) {
			// On JDK 1.5 and higher, it is almost always safe to cache the bean
			// class...
			// The sole exception is a custom BeanInfo class being provided in a
			// non-safe ClassLoader.
			boolean fullyCacheable = ReflectUtil.isCacheSafe(beanClass, CachedIntrospectionResults.class.getClassLoader()) || isClassLoaderAccepted(beanClass.getClassLoader());
			if (fullyCacheable || !CommonClassesUtil.isPresent(beanClass.getName() + "BeanInfo", beanClass.getClassLoader())) {
	 ...
	 *             in case of introspection failure
	 */
	private CachedIntrospectionResults(Class<?> beanClass, boolean cacheFullMetadata) throws BaseException {
		try {
			if (logger.isTraceEnabled()) {
				logger.trace("Getting BeanInfo for class [" + beanClass.getName() + "]");
			}
			beanInfo = Introspector.getBeanInfo(beanClass);
			// Immediately remove class from Introspector cache, to allow for
			// proper
			...
			for (PropertyDescriptor pd : pds) {
				if (Class.class.equals(beanClass) && "classLoader".equals(pd.getName())) {
					// Ignore Class.getClassLoader() method - nobody needs to
					// bind to that
					continue;
				}
				if (logger.isTraceEnabled()) {
					logger.trace("Found bean property '" + pd.getName() + "'" + (pd.getPropertyType() != null ? " of type [" + pd.getPropertyType().getName() + "]" : "")
			...
			throw new BaseException("Failed to obtain BeanInfo for class [" + beanClass.getName() + "]", ex);
		}
	}
	...
	private PropertyDescriptor buildGenericTypeAwarePropertyDescriptor(Class<?> beanClass, PropertyDescriptor pd) {
		try {
			...
			throw new BaseException("Failed to re-introspect class [" + beanClass.getName() + "]", ex);
		}
	}
	...
	PropertyDescriptor getPropertyDescriptor(String name) {
		...
		if ((pd == null) && StringUtil.isNotEmpty(name)) {
			// Same lenient fallback checking as in PropertyTypeDescriptor...
			pd = propertyDescriptorCache.get(name.substring(0, 1).toLowerCase() + name.substring(1));
			if (pd == null) {
				pd = propertyDescriptorCache.get(name.substring(0, 1).toUpperCase() + name.substring(1));
			}
		}

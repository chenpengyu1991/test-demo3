 ...
 * @author liuk
 */
@Repository("dmTumorFollowupDao")
public class DmTumorFollowupDaoImpl extends AbstractDao<DmTumorFollowup, Long> implements IDmTumorFollowupDao {
	@Resource(name = "phbdbJDBCTemplate")
	private SimpleJdbcTemplate simpleJdbcTemplate;

	@Override
	public List<DmTumorFollowup> getUnDeList(Criteria criteria) {
		criteria.add(getHmCardDeleteStatus("DM_DISEASE_INFO.", EHRConstants.DM_MANAGED_FLAG));
		StringBuilder sql = new StringBuilder("SELECT DM_TUMOR_FOLLOWUP.* FROM DM_TUMOR_FOLLOWUP LEFT JOIN DM_DISEASE_INFO ON DM_TUMOR_FOLLOWUP.PERSON_ID=DM_DISEASE_INFO.PERSON_ID ");
		SqlBuilder.buildWhereStatement(DmTumorFollowup.class, sql, criteria);
		return getList(sql.toString(), criteria);
	}

	@Override
	public DmTumorFollowup getLastFollowup(Long personId) {
		String sql = "SELECT A .* FROM (SELECT * FROM DM_TUMOR_FOLLOWUP  WHERE PERSON_ID=:personId  ORDER BY CREATE_DATE DESC)A WHERE ROWNUM=1 ";
        MapSqlParameterSource parameterSource= new MapSqlParameterSource("personId", personId);
        return get(DmTumorFollowup.class, sql, parameterSource);
	}
	...
	public Map<String, Object> getNumGroupByVisitWayCode(Criteria criteria) {
		StringBuilder sql = new StringBuilder("select nvl(sum(case when VISIT_WAY_CODE = '1' then 1 else 0 end), 0) mz_num,\n" +
				...
				"    count(*) hj_num from DM_TUMOR_FOLLOWUP");

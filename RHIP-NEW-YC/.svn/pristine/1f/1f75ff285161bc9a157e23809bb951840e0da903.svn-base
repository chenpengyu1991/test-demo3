...
public abstract class BridgeMethodResolver {
	...
	public static Method findBridgedMethod(Method bridgeMethod) {
		...
		Method[] methods = ReflectUtil.getAllDeclaredMethods(bridgeMethod.getDeclaringClass());
		...
		if (bridgedMethod != null) {
			...
			// A bridge method was passed in but we couldn't find the bridged
			// method.
	...
	private static Method findGenericDeclaration(Method bridgeMethod) {
		...
		// Search interfaces.
		Class<?>[] interfaces = ReflectUtil.getAllInterfacesForClass(bridgeMethod.getDeclaringClass());
	...
	/**
	 * Returns <code>true</code> if the supplied '<code>candidateMethod</code>'
	...
	private static boolean isBridgedCandidateFor(Method candidateMethod, Method bridgeMethod) {
		return (!candidateMethod.isBridge() && !candidateMethod.equals(bridgeMethod) && candidateMethod.getName().equals(bridgeMethod.getName()) && (candidateMethod
	...
	/**
	 * Returns <code>true</code> if the {@link Type} signature of both the
	 * supplied {@link Method#getGenericParameterTypes() generic Method} and
	 * concrete {@link Method} are equal after resolving all
	 * {@link TypeVariable TypeVariables} using the supplied TypeVariable Map,
	...
	private static Method searchForMatch(Class<?> type, Method bridgeMethod) {
		return ReflectUtil.findMethod(type, bridgeMethod.getName(), bridgeMethod.getParameterTypes());
	}
}

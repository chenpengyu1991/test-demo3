...
function showRegister(paramArray)
{
	...
	hrefStr = "<a id='fpRegister' onclick='submitRegister("+param+", true"+")' title='"+paramArray[3]+"' class='finger'>"+paramArray[3]+"</a>";
...
function submitRegister(title, fpCount, saveText, downloadText, isDriverInstall)
{
	...
	else if(!isDriverInstall)
	{
        messageBox({messageType: "confirm", text: "请安装相关硬件驱动！点击确定下载驱动。",
            callback: function(){
                window.location.href = contextPath+"/js/views/ZKTfinger/middleware/setup.exe";
            }});
		/*alert("请安装指纹驱动或启动该服务!");

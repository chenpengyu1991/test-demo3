var edit_require = (function() {
	...
	function initEvent() {
		/*给链接赋予事件*/
		initLinkClick('returnId',returnSearch);
		initLinkClick('passId',approval, {status:"data-status"});
		initLinkClick('cancelId',approval, {status:"data-status"});
		initLinkClick('recordId',approvalDialog, {id:"data-id"});
		initLinkClick('saveSubmit',reportSubmit);
		//report.deleteContent report.infectiousCode
		/*给控件赋予事件*/
		$("#age").change(occupationCheckAge);
		$("#ageUnit").change(occupationCheckAge);
		$("#Occupation").change(function() {
			toggleOccupation('report.occupation','spanOccupationOther','CV020120299');
			occupationCheck(this.value);
		});
		$("#infectiousCode").change(function() {
			infectiousChange(this.value);
		});
		//$('input[name="report.infectedpersonBelong"]').change(toggerAddress);
		$('input[name="report.deleteContent"]').change(function() {
			toggleOther('report.deleteContent','deleteContentOtherId','99');
		});
	}
    /**
...
//    判断初治还是复治
    ...
    function displayPaAddress() {
        ...
        $("select[name='report.pastreet']").on("change villageChange", function()
        {
            //$(this).multiselect('refresh');
            var prefix = $("select[name='report.patownShip']").find("option[value!='']:selected").text();
    ...
    function displayHrAddress() {
        ...
        $("select[name='report.hrstreet']").on("change villageChange", function()
        {
            //$(this).multiselect('refresh');
            var prefix = $("select[name='report.hrtownShip']").find("option[value!='']:selected").text();
            prefix += $(this).find("option[value!='']:selected").text();
...
//        lmhCheck();
	...
	function checkInfection(){
		var infectiousCode = $('#infectiousCode').val();
		if(infectiousCode =='225' || infectiousCode =='2032'){
    ...
    function showControl()
    {
        ...
        toggleOtherSCMul('report.infectiousCode','222223detail','222,2231,2232,2233,2234,2235,202');
...
//        toggleOtherSC('report.infectiousCode','222223detail','2235');//梅毒  隐性
    ...
    function initShowControl()
    {
        ...
        toggleOtherSCByValueMul(value,'222223detail','222,2231,2232,2233,2234,2235,202');
...
//        if('222,2231,2232,2233,2234,2235,202'.indexOf(infectiousCode)!= -1){

var roachMonitorList = (function() {
	...
	function viewOrEdit(monitorId,type, index) {
			...
			$("#mainSearchDiv").hide();
			var option = {
					url : "/dmbc/vertor/initRoachMonitorViewModify/"+monitorId,
	...
	function deleteDo(id){
		$.getJsonByUrl({
	    	url : "/dmbc/vertor/delRoachMonitor",
            callback:function(data){
            	layer.alert(data.message, {icon:0,title:'提示'});
            	if (data.result) {
            		roachMonitorSearch.search($("#currentPage").val());
        		}
    		},
	    	param:{
	    		id:id

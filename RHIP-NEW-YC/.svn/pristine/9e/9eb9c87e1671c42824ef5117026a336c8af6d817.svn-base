...
<div class="repeattable">
	<table class="layui-table x-admin-sm-table-list-middle">
		...
		<tbody>
			<c:forEach var="detail" items="${pharmacyReturnDetails}" varStatus="status">
				<tr>
                    ...
                    <td><tags:numberLabel value="${detail.RETURN_NUM}" fractionDigits="2"/></td>
                    <td><tags:numberLabel value="${detail.RETURN_PRICE}" fractionDigits="2"/></td>
                    <td><tags:numberLabel value="${detail.RETURN_AMOUNT}" fractionDigits="2"/></td>
                    <td><ehr:tip>${detail.BATCH_NO}</ehr:tip></td>
                    <td><ehr:tip><fmt:formatDate value="${detail.EXPIRY_DT}" pattern="yyyy/MM/dd"/></ehr:tip></td>
				</tr>
			</c:forEach>
			<tr>
			<ehr:pagination action="detailSearch.search" colspan="14"/>
		</tr>
		</tbody>
	</table>
	<%-- <table>

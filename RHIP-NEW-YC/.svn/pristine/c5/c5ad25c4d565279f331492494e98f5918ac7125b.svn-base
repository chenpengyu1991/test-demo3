...
@Controller
@RequestMapping(value = "/vaccine/statistics/report/acuteReport")
public class RabiesAcuteReportController extends BaseController {
	...
	public String monthSearch(HttpServletRequest request,ModelMap model) {
		...
		return "rhip.vaccine.statistics.report.acuteReport.monthSearch";
	}	
	/**
	 * 急性传染病防治-按月汇总列表
	 * @param request
	...
	public String monthList(ReportStatisticsQueryForm form,HttpServletRequest request,ModelMap model) {
        ...
        if(ObjectUtil.isNullOrEmpty(orgCode) && hasRole(RoleType.ZXYFJZ,request)){
        ...
        if(!OrgGenreCode.HOSPITAL.getValue().equals(genreCode)){
            ...
            Date endDate = lastDateOfMonth(DateUtil.parseDateString(fillDate.toString() + "/01"));
        ...
        if(ObjectUtil.isNotEmpty(fillOrgan)){
        	Date reportMonthDate = DateUtil.convert("yyy/MM", ca.get("FILL_DATE").toString());
		...
		return "rhip.vaccine.statistics.report.acuteReport.monthList";
	}
	...
	private StatisticsData getStatisticsData(String currentOrgCode,String currentMonth,HttpServletRequest request){
		...
		statisticsData.setFillDate(DateUtil.convert("yyyy/MM/dd", currentMonth + "/01"));
	...
	public String yearSearch(HttpServletRequest request,ModelMap model) {
		...
		model.addAttribute("defaultMonth", new Date());
		return "rhip.vaccine.statistics.report.acuteReport.yearSearch";
	}	
	/**
	 * 急性传染病防治-按年汇总列表
	 * @param request
	...
	public String yearList(ReportStatisticsQueryForm form,HttpServletRequest request,ModelMap model) {
        ...
        Object genreCode = ca.get("GENRE_CODE");//机构类别代码,只有中心需要调用二类疫苗接口
        if(!OrgGenreCode.HOSPITAL.getValue().equals(genreCode)){        
	        Date startDate = firstDateOfMonth(DateUtil.parseDateString(fillDate.toString() + "/01/01"));
	        Date endDate = lastDateOfMonth(DateUtil.parseDateString(fillDate.toString() + "/12/01"));
	        vaccineInterface(startDate,endDate,dtos);
        }
        model.addAttribute("acuteYears", dtos);
		return "rhip.vaccine.statistics.report.acuteReport.yearList";
	}

var hsaFamilyList = (function()
{
	$(function()
	{
		...
		// 点击超链接修改
		$("#hsa-record-family-result").on("click", ".modify-link", function(event)
		{
			event.preventDefault();
			var familyId = $(this).attr("data-record-id");
			if (familyId)
			{
				showList(false);
				var loadHtmlByUrlOption = {
		...
		// 点击超链接修改
		$("#hsa-record-family-result").on("click", ".view-link", function(event)
				{
			event.preventDefault();
			var familyId = $(this).attr("data-record-id");
			if (familyId)
			{
				showList(false);
				var loadHtmlByUrlOption = {
						url : "/hsa/inspRecord/familyRecordView",
						insertDiv : "hsa-record-family-view-box",
						param : {
							id : familyId
						}

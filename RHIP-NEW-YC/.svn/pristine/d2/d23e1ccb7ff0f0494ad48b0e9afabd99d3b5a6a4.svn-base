    ...
    <table class="layui-table x-admin-sm-table-list-middle">
        ...
        <c:forEach var="prenatalFollowup" items="${prenatalFollowups}" varStatus="status">
            <tr>
                ...
                <td class="centertd">
                	<a href="javascript:void(0);"  onclick="womenHealth.viewPrenatalFollowup(${prenatalFollowup.id})" title="查看" class="layui-btn layui-btn-normal layui-btn-xs" style="color: #FFF;font-size: 12px;"><i class="layui-icon">&#xe615;</i>查看</a>
             <%--       <ehr:authorize ifAnyGranted="02,03">
                        <c:if test="${currentLoginInfo.organization.organCode eq prenatalFollowup.createOrganCode}">
                            <a href="javascript:void(0)" onclick="pFPSearch.getPrenatalFollowup(${prenatalFollowup.id})" class="person-link-btn">修改</a>
                            <a href="javascript:void(0)" onclick="pFPSearch.deletePrenatalFollowup(${prenatalFollowup.id})" class="person-link-btn">删除</a>
                        </c:if>
                    </ehr:authorize>--%>
                </td>
            </tr>

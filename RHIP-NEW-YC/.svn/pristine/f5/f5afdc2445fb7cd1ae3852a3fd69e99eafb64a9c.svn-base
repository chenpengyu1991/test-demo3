...
<div>
	...
	<div class="toolbarpop">
	    <c:if test="${type == 'add'}">
			<input type="button" id="saveRestSd" name="saveRestSd" value="添加" onclick="restdrugreg.saveRecord('add')">
	    </c:if>
	    <c:if test="${type == 'edit'}">
			<input type="button" id="modifyRestSd" name="modifyRestSd" value="修改" onclick="restdrugreg.saveRecord('edit')">
	    </c:if>	
		<input type="button" id="cancelRestSd" name="cancelRestSd" value="取消" onclick="idmCommon.closePopUp('restSdDialog')">
	</div>
</div>
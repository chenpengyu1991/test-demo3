...
<div class="repeattable x-admin-sm" >
    <table class="layui-table x-admin-sm-table-list-middle">
        ...
        <c:forEach var="child" items="${childList}" varStatus="status">
            <tr>
                <td class="centertd"><a href="javascript:void(0);"  onclick="childHealth.record('${child.babyCardNo}','${child.idCard}')" >${child.name}</a></td>
                <td class="centertd"><ehr:tip>${child.idCard}</ehr:tip></td>
                <td class="centertd"><ehr:tip>${child.babyCardNo}</ehr:tip></td>
                ...
                <td class="centertd"><ehr:tip><fmt:formatDate value="${child.childBirthday}" pattern="yyyy/MM/dd"/></ehr:tip></td>
                <td class="centertd"><ehr:tip>${child.age}岁</ehr:tip></td>
        	 ...
        	 <tr>
            	<ehr:pagination action="childHealth.search" colspan="13"/>
       		 </tr>
        </tbody>
    </table>
    ...
    <%-- <table>
        <tr>
            <ehr:pagination action="childHealth.search"/>
        </tr>
    </table> --%>
</div>
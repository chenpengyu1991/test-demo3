 ...
 *
 */
@Service("maternalChildrenHospitalReserveAdapter")
public class MaternalChildrenHospitalReserveAdapter  implements IHospitalReserveAdapter {
	...
	protected static Logger logger = Logger.getLogger(MaternalChildrenHospitalReserveAdapter.class.getName());
	...
	public Map<String,Object> queryDepartmentList(){
		...
		String response = Axis2Util.sendService(params, TransCodeConstants.INTERFACE_TRADE_JKZL, Constants.MATERNAL_CHILD_HOSPITAL);
		mapResult = Axis2Util.toXmlResult(response,Constants.MATERNAL_CHILD_HOSPITAL);
		...
		try {
			...
			for (Dept dept : deptmentListResponse.getUpperDept()) {
				...
				outClinic.setHospitalCode(Constants.MATERNAL_CHILD_HOSPITAL);
	...
	public Map<String, Object> querySchduleList(String hospitalcode, Date requestDate, String ampm,
		...
		//webservice发送报文并返回
		String resultXml = Axis2Util.sendService(params,TransCodeConstants.INTERFACE_TRADE_JKZL,Constants.MATERNAL_CHILD_HOSPITAL);
        mapResult = Axis2Util.toXmlResult(resultXml, Constants.MATERNAL_CHILD_HOSPITAL);
        if (Constants.RET_CODE_ERROR.equals(mapResult.get(Constants.RESERVE_RET_CODE))) {
            return mapResult;
        }
        //获取返回的XML
		...
		String hospitalName = hospitalInfoDao.get(new Criteria("HOSPITAL_NO",Constants.MATERNAL_CHILD_HOSPITAL).add("IS_DELETE","0")).getHospitalName();
		if (schResp.getPreInfo() != null) {
			for (PerInfo perInfo : schResp.getPreInfo()) {
				if (!"0".equals(perInfo.getPreLimit()) && StringUtil.isNotEmpty(perInfo.getPreLimit())) {//妇幼预约限号
					...
					registerSchedule.setHospitalCode(Constants.MATERNAL_CHILD_HOSPITAL);
	...
	public Map<String,Object> saveReserve(ReserveRegister reserveRegister){
        ...
        /*Criteria criteria = new Criteria("HOSPITAL_NO",Constants.MATERNAL_CHILD_HOSPITAL);
        criteria.add("ID_CARD",reserveRegister.getIdcard());
        OutReginfo outReginfo = outReginfoDao.get(criteria);*/
        
        /*if (ObjectUtil.isNullOrEmpty(outReginfo)) {*///本地库没有查询到sickId
        	...
        	//接口返回错误情况
        	if(!"0".equals(patientInfoRep.getRespCode())) {//HIS没有查询到患者信息,通过HIS进行无卡预注册
        		mapResult = noCardPreRegister(reserveRegister);
        		if (StringUtil.isNotEmpty(mapResult.get("SickID").toString())) {
        			sickId = mapResult.get("SickID").toString();//通过预注册获取sickId
        		}
        	} else {
        		sickId = patientInfoRep.getSickInfo().getPatientID();//通过HIS查询患者的信息,获取sickId
        	}
        	
        	//将sickId,hospitalNo,idCard存入本地表
        	/*if (StringUtil.isNotEmpty(sickId)) {
            	OutReginfo outreginfo = new OutReginfo();
        		outreginfo.setIdCard(reserveRegister.getIdcard());
        		outreginfo.setHospitalNo(Constants.MATERNAL_CHILD_HOSPITAL);
        		outreginfo.setSickId(sickId);
        		Long id  =outReginfoDao.generatedKey(outreginfo, "ID", null).longValue();
                //保存失败
                if(id == 0 ){
                    mapResult.put(Constants.RESERVE_RET_CODE, Constants.RET_CODE_ERROR);
	...
	* @throws
    ...
    public Map<String, Object> selectSchdule(RegisterSchedule registerSchedule) {
        ...
        for (TimeList num : nums) {
            ...
            regSchedule.setHospitalCode(Constants.MATERNAL_CHILD_HOSPITAL);

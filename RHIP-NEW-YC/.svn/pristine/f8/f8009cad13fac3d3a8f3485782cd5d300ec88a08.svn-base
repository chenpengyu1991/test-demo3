	...
	<tbody>
		<c:forEach var="record" items="${inspectionRecords}" varStatus="status">
			<tr>
				...
				<td><ehr:tip>${record.phoneNumber}</ehr:tip></td>
				...
				<td>
				<a href='javascript:;' data-record-id='${record.id}' class='view-link'>查看</a>
				<a href='javascript:;' data-record-id='${record.id}' class='modify-link'>修改</a></td>
			</tr>
		</c:forEach>
	</tbody>
</table>

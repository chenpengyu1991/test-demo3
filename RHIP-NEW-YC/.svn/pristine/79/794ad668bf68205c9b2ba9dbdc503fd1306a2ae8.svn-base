...
<script src="${pageContext.request.contextPath}/js/views/phsr/hbpCensus/search.js" type="text/javascript"></script>
...
<div class="section">
		<div class="toolbar">
			<div class="x-nav">
		      <span class="layui-breadcrumb">
		        ...
		        <a>
		          <cite>高血压患者健康管理统计报表</cite></a>
		      </span>
		</div>
			<!-- <a id="hbpCensusResultDivExport" href="javascript:void(0)"><b class="export">导出</b></a> -->
			
		</div>
		<div class="searchBox searchSection x-admin-sm">
			<input type="hidden" name="timeFlag" value="1"/>
			<form id="hbpCensusForm">
				<table id="hbpCensus">
					<colgroup>
					...
					<tbody>
						<tr>
							...
							<td class="col-input" colspan="3">
								<input type="radio" id="mhmReprtYearId" name="countType"  class="radioGroup" value="1" checked="checked" onclick="hbpCensus.changeReportType()"/><label for="mhmReprtYearId">按年</label>
                       		 	<input type="radio" id="mhmReprtQuarterId" name="countType"  class="radioGroup" value="2" onclick="hbpCensus.changeReportType()"/><label for="mhmReprtQuarterId">按季度</label>
							...
							<td class="col-input" colspan="4">
								<ehr:authorize ifAnyGranted="01,0107,12">
			...
			<%--					<ehr:authorize ifAnyGranted="04,0407">
									<ehr:dic-org-list name="orgCode" width="150px" isShowOneself="true"/>
								</ehr:authorize>--%>
								<ehr:authorize ifAnyGranted="04,0407,0307">
									<ehr:dic-org-list name="orgCode" width="150px" isShowOneself="true" cssClass="x-layui-input"/>
								</ehr:authorize>
								<ehr:authorize ifAnyGranted="02,0207,03">
								<ehr:dic-org-list name="orgCode" width="150px" isShowOneself="true" cssClass="x-layui-input"/>
								</ehr:authorize>
							</td>
							
                            <td style="text-align: right;"><!-- <input type="button" id="hbpCensusBtnSearch" value="查询"
								 class="search_btn" /> -->
								 <button class="layui-btn layui-btn-sm" id="hbpCensusBtnSearch"><i class="layui-icon">&#xe615;</i>查询</button>
								 </td>
						</tr>
					</tbody>
				</table>
				<table>
					<tr>
						<td colspan="4" class="colbottom"><span
							onclick="hbpCensus.toggle(this,'hbpCensus')"
		...
		<a id="hbpCensusResultDivExport" href="javascript:void(0)"><button class="layui-btn layui-btn-sm" ><i class="layui-icon">&#xe67d;</i>导出</button></a>
		</div>
		<form id="communityInfoForm">
			<div id="hbpCensusResultDiv">

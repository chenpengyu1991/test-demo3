...
<c:choose>
	<c:when test="${message != '' && from eq 'explore' }"><input type="hidden" id="message" value="${message}"/></c:when>
	<c:otherwise>
<div class="toolbar">
	<c:if test="${from != 'explore' }">
		<a href="javascript:void(0)" id="back"><b class="fanhui">返回</b></a>
		<a href="javascript:void(0)" id="save"><b class="baocun">保存</b></a>
	</c:if>
	<c:if test="${from eq 'explore' }">
		<a href="javascript:void(0)" id="saveByexplore"><b class="baocun">保存</b></a>
		...
		<fieldset>
			...
			<table class="posttable">
				...
				<tr>
					...
					<td><input type="text" name="referralDoctorName" value="${!empty referral.referralDoctorName ? referral.referralDoctorName : docName}" style="width: 150px" reg="{'required':'true','maxlength':50}"/></td>

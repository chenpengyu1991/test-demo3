var ndyConfirm = (function() {
	var title = "";
	$(function() { 
        search(1);
        $("#ndyConfirmBtnSearch").click(function(e) {
            e.preventDefault();
        	search(1);
        });
        $("#check-submit-btnNdy1").on("click", function (e) {
            e.preventDefault();
        	StartRead();
        });
        $("#ndySearch").onEnter(search, 1);
    ...
    function StartRead()//开始读卡
    {
        ...
        $("#idCardNdy1").val(GT2ICROCX.CardNo);
    }
    function ndyQz(id){
        /*layer.confirm("确认此人是耐多药患者?", function(index) {
            $.getJsonByUrl({
        ...
        layui.use('layer', function(){
			...
			layer.confirm("确认此人是耐多药患者?", {icon:1, title:'确认提示'}, function(){
				$.getJsonByUrl({
					url : contextPath + "/idm/tb/ndy/confirm/" + id,
					callback : function(data){
						if(data.result){
							layer.closeAll();
							 search(1);
	...
	function search(indexPage) {
		var searchObj = {
				url : "/idm/tb/ndy/list",
				insertDiv : "listDivNdyConfirm",
//                wait : true,
				param : {
					indexPage : indexPage
				}
			};
        $("#ndyConfirmSearchForm").submitFormLoadHtml(searchObj);
	};

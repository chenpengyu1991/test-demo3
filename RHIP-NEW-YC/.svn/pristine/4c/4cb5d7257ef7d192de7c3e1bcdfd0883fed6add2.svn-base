var roachMonitorAdd = (function() {
	...
	/*保存蟑螂监测基本信息*/
	function saveRoachMonitor() {
		var rs =validate.validateForm();
		if(!rs)
			return;
			//参数
			var saveObj = {
				url : "/dmbc/vertor/saveRoachMonitor",
			...
			$("#roachMonitorForm").submitFormLoadHtml(saveObj);
	};
	
	/*进入添加捕蟑螂记录页面*/
	function initRoachAdd(){
		var dialogParams = {
				id : "d1",
				url : "/dmbc/vertor/initRoachAdd/"+$("#monitorId").val(),
				height : 485,
				width : 550,
				title : "添加捕蟑螂记录"
		};
	...
	return {
		...
		saveRoachMonitor:saveRoachMonitor,
		initRoachAdd:initRoachAdd,
		initRoachEdit:initRoachEdit,
		del:del,
		search:search
	};
})();

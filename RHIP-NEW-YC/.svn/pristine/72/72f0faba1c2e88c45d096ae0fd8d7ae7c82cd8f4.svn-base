var statisticsSearch = (function() {
	...
	/*	$('#station2').on("change",function(){
		...
		$("#registerTargetBtnSearch").click(function() {
			search();
		});

		$("#registerTargetSearchForm").onEnter(function() {
			search();
		});
		$("#targetSearchRegSpanId").click(function(){
			toggle(this,'targetSearchReg');
		});
		initForm();
	});
	...
	function search() {
		$("#registerTargetSearchForm").submitFormLoadHtml({
			url : "/protal/statistics/list",
			insertDiv : "resultDiv"
		});
	}

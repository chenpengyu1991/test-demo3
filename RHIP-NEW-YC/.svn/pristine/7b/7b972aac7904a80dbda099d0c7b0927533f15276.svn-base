...
<div style="background-color: white; height: 515px;">
    <ul>
        <li style="text-align: center; font-size: 25px;">1-8月龄儿童健康检查记录表</li>
    </ul>
    <br/>
    <table>
        <colgroup>
    ...
    <div class="table-basic">
        <table>
            ...
            <tr>
                ...
                <td >
                    <c:if test="${!empty exam.skinInspectionAnomalySign}">
                        <c:out value='${exam.skinInspectionAnomalySign eq "1" ? "未见异常" : "有异常"}'/>
            ...
            <tr>
                <th>颈部包块</th>
                <td>
                    <c:if test="${!empty exam.neckLumpSign}">
                        <ehr:dic code="${exam.neckLumpSign}" dicmeta="PH00002"/>
                    </c:if>
                </td>
                <th>眼睛检查异常</th>
                <td>
                    <c:if test="${!empty exam.eyeappearanceSign}">
                        <c:out value='${exam.eyeappearanceSign eq "1" ? "未见异常" : "有异常"}'/>
            ...
            <tr>
                ...
                <td>
                    <c:if test="${!empty exam.earappearanceSign}">
                        <c:out value='${exam.earappearanceSign eq "1" ? "未见异常" : "有异常"}'/>
            ...
            <tr>
                <th>口腔检查异常</th>
                <td>
                    <c:if test="${!empty exam.oralExamination}">
                        <c:out value='${exam.oralExamination eq "1" ? "未见异常" : "有异常"}'/>
                        <c:if test="${exam.oralExamination ne '1'}">
                            <br/><c:out value="${exam.oralExaminationDesc}"/>
                        </c:if>
                    </c:if>
                </td>
                <th>出牙数(颗)</th>
            ...
            <c:if test="${exam.cPhysicalExamAge eq '满月'}">
            <tr>
                <th>脐带检查结果</th>
                <td>
                    <ehr:dic code="${exam.umbilicalCordCheck}" dicmeta="CV0410019"/>
                </td>
            </tr>
            </c:if>
            <c:if test="${exam.cPhysicalExamAge eq '3月龄'}">
                <tr>
                    <th>脐带检查结果</th>
                    <td>
                        <ehr:dic code="${exam.umbilicalCordCheck}" dicmeta="FS10046"/>&nbsp;&nbsp;&nbsp;
                        <c:out value="${exam.umbilicalCordCheckDesc}"/>
                    </td>
                </tr>
            </c:if>
            <tr>
                ...
                <td colspan="3">
                    <c:if test="${!empty exam.limbActivityAnomalySign}">
                        <c:out value='${exam.limbActivityAnomalySign eq "1" ? "未见异常" : "有异常"}'/>
            ...
            <tr>
                ...
                <th>可疑佝偻病症状</th>
                <td>
                    <ehr:dic code="${exam.suspiciousRicketsSymptoms}" dicmeta="CV0410021"/>
                </td>
            </tr>
            <tr>
                <th>肛门/外生殖器检查异常</th>
                <td>
                    <c:if test="${!empty exam.analGenitaliaCheck}">
                        <c:out value='${exam.analGenitaliaCheck eq "1" ? "未见异常" : "有异常"}'/>
                        <c:if test="${exam.analGenitaliaCheck ne '1'}">
                            <br/><c:out value="${exam.analGenitaliaAnomalyDesc}"/>
                        </c:if>
                    </c:if>
                </td>
                <th>血红蛋白值(g/L)</th>

...
<div>
	<form id="addLe2Form" method="get">
		<div>
			<table class="formtable" id="popLe2Table">
				<tr>
                    <input type="hidden" id="rowIndex" value="${rowIndex}"/>
					<th style="width: 25%">检查时间</th>
					<td>
                        <tag:dateInput name="xDt" onlypast="true" date="${idmListLe.xDt}" reg='{"required":"true"}'/>
                    </td>
    ...
    <div class="toolbarpop">
        <c:if test="${type == 'add'}">
            <input type="button" id="saveContact" value="添加" onclick="hiwhpaiCase.saveLeData('add',2)">
        </c:if>
        <c:if test="${type == 'edit'}">
            <input type="button" id="editContact" value="修改" onclick="hiwhpaiCase.saveLeData('edit',2)">
        </c:if>

...
public class MethodInvoker {
	...
	public static int getTypeDifferenceWeight(Class<?>[] paramTypes, Object[] args) {
		...
		for (int i = 0; i < paramTypes.length; i++) {
			if (!CommonClassesUtil.isAssignableValue(paramTypes[i], args[i])) {
			...
			if (args[i] != null) {
				...
				while (superClass != null) {
					if (paramType.equals(superClass)) {
						result = result + 2;
						superClass = null;
					} else if (CommonClassesUtil.isAssignable(paramType, superClass)) {
						result = result + 2;
	...
	protected Method findMatchingMethod() {
		...
		Method[] candidates = ReflectUtil.getAllDeclaredMethods(getTargetClass());
	...
	public Object invoke() throws InvocationTargetException, IllegalAccessException {
		...
		ReflectUtil.makeAccessible(preparedMethod);
	...
	protected Class<?> resolveClassName(String className) throws ClassNotFoundException {
		return CommonClassesUtil.forName(className, CommonClassesUtil.getDefaultClassLoader());
	}

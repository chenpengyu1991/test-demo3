...
public class ChainedPropertyAccessor implements PropertyAccessor {
	...
	public Getter getGetter(Class<?> theClass, String propertyName)
		...
		for (int i = 0; i < chain.length; i++) {
			...
			try {
				...
				return result;
			} catch (BaseException pnfe) {
				// ignore
			}
		}
		throw new BaseException("Could not find getter for " + propertyName + " on " + theClass);
	}
	...
	public Setter getSetter(Class<?> theClass, String propertyName)
		...
		throw new BaseException("Could not find setter for " + propertyName + " on " + theClass);
	}

...
public class ComparisonVector implements Serializable {
	...
	public void setScore(int index, double value) {
		if (index < 0 || index > scores.length - 1) {
			throw new IndexOutOfBoundsException("Index value of " + index
					+ " outside valid range of 0 - " + (scores.length - 1));
		}
		scores[index] = value;
	...
	private double calculateVectorProbability(double[] estimatedMarginals) {
		if (estimatedMarginals.length == 0
				|| estimatedMarginals.length != binaryScores.length) {
			...
			throw new RuntimeException(
					"Unable to calculate vector marginal probabilities.");
		}
		double product = 1.0;
		for (int i = 0; i < binaryScores.length; i++) {
			if (binaryScores[i] == 0) {
	...
	public String toString() {
		return new ToStringBuilder(this).append("matchFields", matchFields)
				.append("scores", scores).append("binaryScores", binaryScores)

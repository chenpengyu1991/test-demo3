 ...
 * "back-reference" is pertinent in mapping scenarios where we have a
 * uni-directional one-to-many association in which only the many side is
 * mapped. In this case it is the collection itself which is responsible for the
 * FK value.
 * <p/>
 * In this scenario, the one side has no inherent knowledge of its "owner". So
 * we introduce a synthetic property into the one side to represent the
 * association; a so-called back-reference.
 * 
 * @author Gavin King
...
public class BackrefPropertyAccessor implements PropertyAccessor {
	...
	/**
	 * A placeholder for a property value, indicating that we don't know the
	 * value of the back reference
	 */
	public static final Serializable UNKNOWN = new Serializable() {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 8311110712939246823L;
	...
	/**
	 * Internal implementation of a property setter specific to these back-ref
	 * properties.
	 */
	public static final class BackrefSetter implements Setter {
		 ...
		 * 
		 */
		private static final long serialVersionUID = -4358652148823174818L;
	...
	/**
	 * Internal implementation of a property getter specific to these back-ref
	 * properties.
	 */
	...
	public class BackrefGetter implements Getter {
		 ...
		 * {@inheritDoc}
		 */
		public Object getForInsert(Object target, @SuppressWarnings("rawtypes") Map mergeMap) {
			// if ( session == null ) {
			return UNKNOWN;
			// }
			// else {
			// return session.getPersistenceContext().getOwnerId( entityName,
			// propertyName, target, mergeMap );
			// }
		}
